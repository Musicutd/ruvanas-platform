...
export default function IntegrationConsole({ organisations, initialConnections }) {
  ...
  async function create(event) {
    ...
    try {
      ...
      setConnections((items) => [{ ...body.connection, organisation, _count: { events: 0, syncRuns: 0, metricSummaries: 0 }, deliveryHealth: {}, events: [], syncRuns: [] }, ...items]);
  ...
  async function dispatch(id, action = "DELIVER_DUE") {
    setWorking(true); setNotice("");
    try {
      const body = await callApi(`/api/admin/integrations/connections/${id}/dispatch`, { method: "POST", body: JSON.stringify(action === "RECOVER_ABANDONED" ? { action, note: recoveryNotes[id] || "" } : { action }) });
      setConnections((items) => items.map((item) => item.id === id ? { ...item, deliveryHealth: body.deliveryHealth, events: body.events } : item));
      if (action === "RECOVER_ABANDONED") {
        setRecoveryNotes((items) => ({ ...items, [id]: "" }));
        setNotice(`${body.recovered} abandoned event(s) queued for controlled recovery. The original attempt history was retained.`);
      } else setNotice(`Delivery run complete: ${body.delivered} of ${body.attempted} due events delivered; ${body.workerErrors || 0} isolated worker error(s).`);
      setWorking(false);
    }
    catch (error) { setNotice(error.message); setWorking(false); }
  }
  ...
  return <div style={s.grid}>
    ...
    <section style={s.wide}><p style={s.eyebrow}>Connection status</p><h2 style={s.h2}>Managed integrations</h2>
      {!connections.length ? <p style={s.muted}>No connections yet. Create one when a trusted partner provides its HTTPS endpoint.</p> : connections.map((item) => <article key={item.id} style={s.item}>
        ...
        <p style={s.small}>{item.kind === "OUTGOING_WEBHOOK" ? `${item.subscribedEventTypes.join(" · ")} · ${item._count.events} delivery history items` : `${item._count.metricSummaries || 0} accepted summaries · ${item._count.syncRuns || 0} import runs`}</p>
        {item.kind === "OUTGOING_WEBHOOK" ? <div style={s.health}>{["PENDING", "FAILED", "DELIVERED", "ABANDONED"].map((status) => <span key={status} style={s.healthItem}><strong>{item.deliveryHealth?.[status] || 0}</strong> {status.toLowerCase()}</span>)}</div> : null}
        ...
        {item.kind === "OUTGOING_WEBHOOK" && (item.deliveryHealth?.ABANDONED || 0) > 0 && (item.status === "CONNECTED" || item.status === "DEGRADED") ? <div style={s.recovery}><label style={s.label}>Recovery reason<textarea style={s.input} minLength="8" maxLength="500" value={recoveryNotes[item.id] || ""} onChange={(event) => setRecoveryNotes((notes) => ({ ...notes, [item.id]: event.target.value }))} placeholder="Explain why these events are safe to retry." /></label><button disabled={working || (recoveryNotes[item.id] || "").trim().length < 8} onClick={() => dispatch(item.id, "RECOVER_ABANDONED")}>Queue abandoned recovery</button><p style={s.small}>Up to 10 events per action. Maximum three recovery cycles per event. Previous attempts remain immutable.</p></div> : null}
        {item.events.length ? <details><summary style={s.summary}>Recent deliveries</summary>{item.events.map((event) => <p key={event.id} style={s.small}>{event.eventType} · {event.status} · {event.attemptCount} attempt(s) · {event.recoveryCount || 0} recovery cycle(s){event.lastError ? ` · ${event.lastError}` : ""}</p>)}</details> : null}
