...
export default async function NewMusicSchedulePage() {
  ...
  const [organisations, locations, modes, radioStations] = await Promise.all([
    ...
    prisma.station.findMany({ where: { productFamily: "ONLINE" }, select: { id: true, organisationId: true, name: true }, orderBy: { name: "asc" } })
  ]);
  return <NewMusicScheduleForm organisations={organisations} locations={locations} modes={modes} radioStations={radioStations} />;
}
