...
export default function RetailMediaEntitlementControl({ organisationId, effectiveEnabled, overrideEnabled, planDefaultEnabled, canManage }) {
  ...
  async function update(enabled) {
    ...
    try {
      const response = await fetch(`/api/admin/organisations/${organisationId}/retail-media`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ enabled }) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "Unable to update Retail Media access.");
      router.refresh();
    } catch (updateError) { setError(updateError instanceof Error ? updateError.message : "Unable to update Retail Media access."); }
