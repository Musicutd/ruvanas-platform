...
export default function RetailMediaConsole({ organisations, canApprove = true, showOrganisationSelector = true }) {
  ...
  function targetOptions(type) {
    ...
    if (type === "STATION") return organisation?.stations || [];
    if (type === "CHANNEL") return (organisation?.channels || []).map((channel) => ({ id: channel.id, name: `${channel.station?.name || "Station"} / ${channel.name}` }));
