...
export default function RetailMediaConsole({ organisations, canApprove = true, showOrganisationSelector = true }) {
  ...
  const [orderForm, setOrderForm] = useState({ name: "", advertiserId: "", agencyId: "", inventoryPackageId: "", campaignId: "", creativePromoVersionId: "", purchaseOrderReference: "" });
  ...
  return <main style={styles.page}>
    <p style={styles.eyebrow}>Stage 7A</p><h1 style={styles.title}>Retail media foundation</h1>
    <p style={styles.copy}>Build subscriber-approved advertiser inventory and audio campaign orders. Advertisers and agencies are commercial records only; they cannot sign in or publish directly.</p>
    ...
    <section style={styles.panel}><h2 style={styles.heading}>3. Supplier campaign orders</h2><div style={styles.grid}>
      ...
      <label style={styles.label}>Creative<select value={orderForm.creativePromoVersionId} onChange={(event) => setOrderForm({ ...orderForm, creativePromoVersionId: event.target.value })} style={styles.input}><option value="">Choose approved audio</option>{promoVersions.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label>
    ...
    </div><button disabled={working || !organisation?.retailMediaEnabled} onClick={async () => { const result = await submit("/api/admin/retail-media/orders", { ...orderForm, organisationId, creativePromoVersionIds: [orderForm.creativePromoVersionId] }, "Order draft created. Submit it for subscriber approval when ready."); if (result) setOrderForm({ name: "", advertiserId: "", agencyId: "", inventoryPackageId: "", campaignId: "", creativePromoVersionId: "", purchaseOrderReference: "" }); }} style={styles.primary}>Create order draft</button>
    <p style={styles.notice}>Audio plays remain device-confirmed delivery events. They are not listeners, viewers, impressions, or proof that media caused an operational outcome.</p>
    <div style={styles.cards}>{orders.map((order) => <article key={order.id} style={styles.orderCard}><div><strong>{order.name}</strong><div style={styles.muted}>{order.advertiser.name} · {order.inventoryPackage.name}{order.agency ? ` · ${order.agency.name}` : ""}</div></div><Badge value={order.status} />{order.creatives.map((creative) => <div key={creative.id} style={styles.creative}><span>{creative.promoVersion.promoAsset.name} v{creative.promoVersion.version}</span><Badge value={creative.status} />{canApprove && order.status === "SUBMITTED" && creative.status === "PENDING" ? <><button disabled={working} onClick={() => patch(`/api/admin/retail-media/orders/${order.id}/review`, { action: "APPROVE_CREATIVE", creativeId: creative.id }, "Creative approved.")} style={styles.secondary}>Approve creative</button><button disabled={working} onClick={() => patch(`/api/admin/retail-media/orders/${order.id}/review`, { action: "REJECT_CREATIVE", creativeId: creative.id }, "Creative rejected.")}>Reject</button></> : null}</div>)}<div style={styles.actions}>{order.status === "DRAFT" ? <button disabled={working} onClick={() => patch(`/api/admin/retail-media/orders/${order.id}/review`, { action: "SUBMIT_ORDER" }, "Order submitted for subscriber approval.")} style={styles.primary}>Submit for approval</button> : null}{canApprove && order.status === "SUBMITTED" ? <><button disabled={working || order.creatives.some((item) => item.status !== "APPROVED")} onClick={() => patch(`/api/admin/retail-media/orders/${order.id}/review`, { action: "APPROVE_ORDER" }, "Subscriber approval recorded. Existing campaign publication checks still apply.")} style={styles.primary}>Approve order</button><button disabled={working} onClick={() => patch(`/api/admin/retail-media/orders/${order.id}/review`, { action: "REJECT_ORDER" }, "Order rejected.")}>Reject order</button></> : null}</div></article>)}</div></section>
  </main>;
}
