...
import {
  adjustSelection, changeSelectionGain, clipDuration, copySelection, deleteSelection, duplicateSelection, MARKER_TYPES, pasteSelection,
...
export default function WaveformEditorClient({ requestedProjectId = "", experienceMode, onExperienceModeChange, apiBase = "/api/school-radio/audio-lab", mediaBase = "/api/media", supervised = false }) {
  ...
  const supervisedRender = supervised && history.length === 0 && future.length === 0
    ? editor?.renders?.find((render) => render.status === "SUCCEEDED" && render.versionNumber === editor.currentVersion && render.streamUrl)
    : null;
  ...
  const selectionDurationMs = Math.abs(selection.endMs - selection.startMs);
  const selectedClips = useMemo(() => state.clips.filter((clip) =>
    clip.timelineStartMs < Math.max(selection.startMs, selection.endMs) &&
    clip.timelineStartMs + clipDuration(clip) > Math.min(selection.startMs, selection.endMs))
    .sort((left, right) => left.timelineStartMs - right.timelineStartMs), [state.clips, selection.startMs, selection.endMs]);
  const fadeLimitMs = Math.min(60000, selectionDurationMs);
  const selectedFadeInMs = hasSelection ? Math.min(fadeLimitMs, selectedClips[0]?.fadeInMs || 0) : 0;
  const selectedFadeOutMs = hasSelection ? Math.min(fadeLimitMs, selectedClips.at(-1)?.fadeOutMs || 0) : 0;
  ...
  return <section ref={sectionRef} style={s.panel} aria-labelledby="waveform-title">
    ...
    {state.clips.length ? <>
      ...
      {supervised && !supervisedRender ? <p style={s.hint}>To hear timeline edits, save and create a review render. The original take stays unchanged.</p> : null}
      {sourceTake ? <audio ref={audioRef} src={supervisedRender?.streamUrl || (supervised ? `${mediaBase}/${sourceTake.mediaAsset.id}` : `${mediaBase}/${sourceTake.mediaAsset.id}/stream`)} onTimeUpdate={onAudioTime} onEnded={() => setCursorMs(0)} preload="metadata" /> : null}
      ...
      {advanced ? <div style={s.advanced}>
        ...
        <div style={s.timeGrid}><label style={s.label}>Fade-in handle<input aria-label="Fade-in handle" type="range" min="0" max={fadeLimitMs} step="10" value={selectedFadeInMs} disabled={!hasSelection || !canEdit} onChange={(event) => editClips((clips) => adjustSelection(clips, selection.startMs, selection.endMs, { fadeInMs: Number(event.target.value) }))} /></label><label style={s.label}>Fade-out handle<input aria-label="Fade-out handle" type="range" min="0" max={fadeLimitMs} step="10" value={selectedFadeOutMs} disabled={!hasSelection || !canEdit} onChange={(event) => editClips((clips) => adjustSelection(clips, selection.startMs, selection.endMs, { fadeOutMs: Number(event.target.value) }))} /></label></div>
