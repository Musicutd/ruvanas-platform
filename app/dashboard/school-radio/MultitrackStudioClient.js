  ...
  return <section id="multitrack-studio" style={s.panel}>
    ...
    {project ? <><div style={s.transport}>
      ...
      <div style={s.timeline}>{project.state.tracks.map((track) => <article key={track.clientId} style={s.track}>
        <div style={s.trackHeader}><input aria-label="Track name" style={{ ...s.input, fontWeight: 900 }} value={track.name} disabled={track.locked} onChange={(event) => updateTrack(track.clientId, { name: event.target.value })} /><select aria-label="Track kind" style={s.compact} value={track.kind} disabled={track.locked} onChange={(event) => updateTrack(track.clientId, { kind: event.target.value })}>{trackKinds.map((kind) => <option key={kind}>{kind}</option>)}</select><button style={track.muted ? s.toggleOn : s.toggle} onClick={() => updateTrack(track.clientId, { muted: !track.muted })}>M</button>{studioPro ? <><button style={track.solo ? s.toggleOn : s.toggle} onClick={() => updateTrack(track.clientId, { solo: !track.solo })}>S</button><button style={track.armed ? s.recordOn : s.toggle} onClick={() => updateTrack(track.clientId, { armed: !track.armed })}>Arm</button><button style={track.locked ? s.toggleOn : s.toggle} onClick={() => updateTrack(track.clientId, { locked: !track.locked })}>Lock</button></> : null}<button style={s.danger} disabled={track.locked || project.state.tracks.length <= 1} onClick={() => updateState((state) => ({ ...state, tracks: state.tracks.filter((item) => item.clientId !== track.clientId) }))}>Remove track</button></div>
        <div style={s.mixer}>{studioPro ? <><label style={s.inline}>Track Gain <input type="range" min="-36" max="12" step="0.5" value={track.gainDb} disabled={track.locked} onChange={(event) => updateTrack(track.clientId, { gainDb: Number(event.target.value) })} /><span>{track.gainDb} dB</span></label><label style={s.inline}>Pan <input type="range" min="-1" max="1" step="0.05" value={track.pan} disabled={track.locked} onChange={(event) => updateTrack(track.clientId, { pan: Number(event.target.value) })} /><span>{track.pan}</span></label><label style={s.inline}>Gain warning <meter min="-36" low="-12" high="0" max="12" value={Math.max(-36, Math.min(12, Number(track.gainDb || 0) + Math.max(0, ...track.clips.map((clip) => Number(clip.gainDb || 0)))))} />{Number(track.gainDb || 0) + Math.max(0, ...track.clips.map((clip) => Number(clip.gainDb || 0))) > 0 ? <span style={s.failure}>May clip</span> : <span>Safe</span>}</label></> : null}<label style={s.inline}>Preset <select style={s.compact} value={track.preset} disabled={track.locked} onChange={(event) => updateTrack(track.clientId, { preset: event.target.value })}>{presets.map((preset) => <option key={preset}>{preset.replaceAll("_", " ")}</option>)}</select></label></div>
