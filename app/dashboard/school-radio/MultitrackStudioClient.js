...
function ClipWaveform({ source, clip, colour }) {
  ...
  return <svg aria-hidden="true" viewBox={`0 0 ${bars.length} 40`} preserveAspectRatio="none" style={s.clipWaveform}>
    {bars.map((peak, index) => <rect key={index} x={index + 0.1} y={20 - peak * 18} width="0.65" height={peak * 36} fill={colour} opacity="0.75" />)}
  </svg>;
}
...
export default function MultitrackStudioClient({ requestedProjectId = "", experienceMode = "BEGINNER", onExperienceModeChange, onOpenRecordings }) {
  ...
  return <section id="multitrack-studio" style={s.panel}>
    <div style={s.heading}><div><p style={s.eyebrow}>MULTITRACK · STUDIO {catalogue.studioLevel}</p><h2 style={s.title}>Build a complete production</h2><p style={s.hint}>Layer voice, music and effects without changing source recordings. Server rendering and loudness checks create the authoritative final version.</p></div>{project ? <div style={s.mode}><button type="button" style={project.state.mode === "BEGINNER" ? s.active : s.secondary} onClick={() => { updateState((state) => ({ ...state, mode: "BEGINNER" })); onExperienceModeChange?.("BEGINNER"); }}>Basic tools</button><button type="button" disabled={!studioPro} title={!studioPro ? "Studio Pro is included with Tiers 3–5." : undefined} style={project.state.mode === "ADVANCED" ? s.active : s.secondary} onClick={() => { updateState((state) => ({ ...state, mode: "ADVANCED" })); onExperienceModeChange?.("ADVANCED"); }}>Pro tools</button></div> : null}</div>
    ...
    {project ? <><div style={s.transport}>
      ...
      <button type="button" style={s.primary} disabled={working || project.restrictedReadOnly} onClick={() => sendAction("SAVE", { reason: "Multitrack manual save" })}>Save snapshot</button>
      <button type="button" style={s.primary} disabled={working || project.restrictedReadOnly} onClick={() => sendAction("QUEUE_RENDER", { preset: "SCHOOL_RADIO_MP3" })}>Render final MP3</button>
    </div>
        ...
        <div style={s.laneScroll}>
          ...
          >
            ...
            {!track.clips.length ? <p style={s.emptyLane}>Drop audio here or use Add clip above.</p> : track.clips.map((clip) => { const source = sourceMap.get(clip.mediaAssetId); const locked = track.locked || clip.locked; return <button
              ...
              style={{ ...s.timelineClip, borderColor: selectedClipId === clip.clientId ? "#fff" : trackColours[track.kind] || trackColours.MIXED, left: (clip.timelineStartMs / 1000) * timelineScale, width: Math.max(72, ((clip.sourceEndMs - clip.sourceStartMs) / 1000) * timelineScale), cursor: locked ? "not-allowed" : "grab" }}
    ...
    <style>{"@media (max-width: 780px) { .rv-multitrack-row { grid-template-columns: minmax(0, 1fr) !important; } }"}</style>
  </section>;
}
...
const s = {
  ...
  laneScroll: { overflowX: "auto", border: "1px solid #44536a", borderRadius: 9, background: "#101722", marginBottom: 10 },
  lane: { position: "relative", height: 126, minWidth: "100%", backgroundImage: "repeating-linear-gradient(90deg, transparent 0, transparent 31px, rgba(148,163,184,.12) 32px), linear-gradient(180deg, rgba(56,189,248,.05), transparent 44%, rgba(244,185,66,.04))" },
  ...
  timelineClip: { position: "absolute", top: 35, height: 72, overflow: "hidden", display: "grid", alignContent: "center", gap: 2, textAlign: "left", border: "1px solid #f4b942", borderRadius: 7, backgroundColor: "#183747", color: "#f8fafc", padding: "6px 9px", boxShadow: "0 4px 12px rgba(0,0,0,.3)" },
  ...
  track: { display: "grid", gridTemplateColumns: "minmax(230px,290px) minmax(0,1fr)", border: "1px solid #41516b", borderLeft: "6px solid #60a5fa", borderRadius: 10, background: "#0f1724", overflow: "hidden", boxShadow: "0 7px 22px rgba(0,0,0,.16)" },
  trackConsole: { padding: 12, borderRight: "1px solid #334155", background: "linear-gradient(180deg,#1a2434,#121a27)" },
  ...
  clipInspector: { border: "1px solid #334155", borderRadius: 8, background: "#111827", padding: 10 },
