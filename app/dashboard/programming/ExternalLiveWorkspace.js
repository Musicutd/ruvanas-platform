...
export default function ExternalLiveWorkspace() {
  ...
  return <section className={styles.panel} aria-labelledby="external-live-title">
    ...
    {sources.length ? <div className={styles.externalLiveGrid}>{sources.map((source) => <article className={source.status === "ACTIVE" ? styles.externalLiveActive : styles.externalLiveCard} key={source.id}>
      ...
      {data.canManage ? <div className={styles.cardActions}><button type="button" className={styles.secondaryButton} disabled={busy !== ""} onClick={() => act(source, "PROBE")}>Test connection</button>{source.status !== "ACTIVE" && source.healthStatus === "HEALTHY" ? <button type="button" className={styles.primaryButton} disabled={busy !== ""} onClick={() => act(source, "ACTIVATE")}>Take live</button> : null}{source.status === "ACTIVE" ? <button type="button" className={styles.removeButton} disabled={busy !== ""} onClick={() => act(source, "SUSPEND")}>Take off air</button> : null}<button type="button" className={styles.secondaryButton} disabled={busy !== "" || source.status === "ACTIVE"} onClick={() => act(source, "ARCHIVE")}>Archive</button></div> : null}
    ...
    </form> : <div className={styles.readOnlyMessage}>You can inspect External Live status. An organisation owner or manager controls credentials and activation.</div>}
  </section>;
}
