...
function StaffGrants({ facilityId, setNotice }) {
  ...
  const change = async (method, selectedId) => {
    setBusy(true);
    try {
      const response = await fetch(`/api/corrections/facilities/${facilityId}/staff`, { method, headers: { "Content-Type": "application/json" }, body: JSON.stringify({ memberId: selectedId, permission }) });
      const result = await response.json();
