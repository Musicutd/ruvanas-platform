...
import { studioTimingToNextHardEvent } from "@/lib/studio-console.mjs";
...
export default function BroadcastConsoleClient({ enabled, monitor = false }) {
  ...
  const visiblePanels = new Set(data?.layout?.panels || []);
  const panelWidths = (keys) => Object.fromEntries(keys.map((id, index) => [`--panel-${index + 1}-width`, `${data?.layout?.sizes?.[id] || 320}px`]));
  const savePanels = (panels, sizes = data?.layout?.sizes || {}) => save("SAVE_LAYOUT", { preset: data.layout.preset, panels, sizes });
  ...
  return <section className={styles.panel}>
    ...
    {!monitor ? <><div className={styles.actions}><label>Layout <select aria-label="Console layout" value={data.layout.preset} disabled={busy} onChange={(event) => save("SAVE_LAYOUT", { preset: event.target.value })}>{presets.map((preset) => <option key={preset} value={preset}>{pretty(preset)}</option>)}</select></label><a href="/dashboard/studio?workspace=playout" className={styles.secondary}>Open Manual Playout controls</a><a href="/dashboard/studio?workspace=broadcast" className={styles.secondary}>Open distribution controls</a></div><details className={styles.layoutControls}><summary>Choose visible panels and widths</summary><p className={styles.muted}>On-air status and output health always stay visible. Changes affect only your Console view, not audio.</p><div className={styles.panelSettings}>{panelOptions.map(([id, label]) => <div className={styles.panelSetting} key={id}><label><input type="checkbox" checked={visiblePanels.has(id)} disabled={busy || id === "ON_AIR" || id === "OUTPUT_HEALTH"} onChange={(event) => savePanels(event.target.checked ? [...data.layout.panels, id] : data.layout.panels.filter((panelId) => panelId !== id))} />{label}</label><label>Width <select aria-label={`${label} width`} value={data.layout.sizes?.[id] || 320} disabled={busy || !visiblePanels.has(id)} onChange={(event) => savePanels(data.layout.panels, { ...data.layout.sizes, [id]: Number(event.target.value) })}>{[240, 320, 480, 640, 800, 1200].map((width) => <option key={width} value={width}>{width}px</option>)}</select></label></div>)}</div></details></> : null}
    ...
    <div className={styles.consoleGrid} data-section="primary" data-panels={monitor ? "ON_AIR DAILY_LOG OUTPUT_HEALTH" : data.layout.panels.join(" ")} style={panelWidths(["ON_AIR", "DAILY_LOG", "OUTPUT_HEALTH"])}>
    ...
    {!monitor ? <div className={styles.consoleGrid} data-section="tools" data-panels={data.layout.panels.join(" ")} style={panelWidths(["PREPARE", "CARTS", "RADIO_CLOCKS", "NOTES", "MIX_POINTS"])}>
