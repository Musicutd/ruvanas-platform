...
export default async function ListenerRequestsPage({ params }) {
  ...
  return <main className={styles.page}><section className={styles.shell}>
    ...
    <p className={styles.intro}>{productKey === "HEALTH" ? "Review privacy-minimal song and artist suggestions. Do not add patient, clinical or treatment information." : "Review song suggestions before they influence any editorial decision. Approved requests remain separate from the live schedule until your team chooses and clears the music."}</p>
