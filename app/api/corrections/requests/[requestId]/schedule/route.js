...
import { scheduleCorrectionsRequest } from "@/lib/corrections-delivery-service";

export async function POST(request, { params }) {
  const access = await correctionsRequestContext();
  if (!access.ok) return correctionsResponse(access);
  try { const { requestId } = await params; return correctionsResponse({ ok: true, delivery: await scheduleCorrectionsRequest(access, requestId, await request.json()) }); }
