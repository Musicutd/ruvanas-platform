...
async function dailyLog(organisationId, channelId, productFamily, requestedDate) {
  ...
  const [generated, campaigns, live, manual, proof] = await Promise.all([
    prisma.generatedPlaylist.findMany({ where: { organisationId, targetId: channelId, status: { in: ["PUBLISHED", "DRAFT"] }, publishedVersion: { gt: 0 }, publishedAt: { not: null }, scheduledDate: new Date(`${date}T00:00:00Z`) }, include: { versions: { where: { publishedAt: { not: null } }, orderBy: { version: "desc" }, take: 1, include: { items: { orderBy: { position: "asc" }, include: { track: { select: { title: true, artist: true } } } } } } }, take: 20 }),
