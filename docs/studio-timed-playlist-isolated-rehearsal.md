...
For a repeatable self-owned test fixture, run `node scripts/prepare-studio-timed-rehearsal.mjs <new-absolute-local-directory>` from this checkout. The directory must not already exist. This produces two distinct MP3 tones, an A-B-A `frozen.m3u`, a **file-output-only** `rehearsal.liq`, and `manifest.json` with the expected offsets, Linux target directory and SHA-256 hashes. The preparation command does not use the database, R2 or Centova and does not run Liquidsoap. Copy those five files to the exact `targetDirectory` named in the manifest on a separate isolated Linux test machine, create that directory with private permissions, and confirm the copied file hashes before executing the script. Never put source credentials or customer audio in this pack.

For an earlier local check, run `node scripts/verify-studio-timed-surrogate.mjs <absolute-local-pack-directory>`. It first verifies the staged hashes, exact frozen order and file-only script. FFmpeg then reads only the two local MP3s, writes `ffmpeg-surrogate.mp3` and a decoded `.s16le` file without overwriting existing files, and applies the existing tone analyser. You can listen to that MP3, but the reported provenance is `LOCAL_FFMPEG_SURROGATE_NOT_LIQUIDSOAP`: a pass proves only the local surrogate chain, **not** that Liquidsoap rendered the template, Centova received it, or a listener heard it. Use a fresh pack to repeat the check.

On Windows, the narrower real-Liquidsoap file check is `node scripts/run-studio-timed-liquidsoap-local.mjs <fresh-absolute-local-pack-directory> <absolute-path-to-liquidsoap-2.1.3.exe>`. Use a fresh pack because the helper will not overwrite prior outputs. It verifies the self-owned fixture, then checks and renders a path-adjusted file-only script. The [official Liquidsoap 2.1.3 Windows release](https://github.com/savonet/liquidsoap-release-assets/releases/tag/v2.1.3) used in the local checkpoint did not support the template's MP3 encoder. In that case the helper substitutes `%wav` **only in the disposable Windows test script** and reports `MP3_TO_WAV_WINDOWS_TEST_ONLY` and `mp3TemplateVerified: false`; the repository's MP3 template is unchanged. The helper accepts a finite-output stop only after Liquidsoap logs that no tracks remain and a nonempty output file exists. Its decoded local-tone result can validate ordering and coarse overlaps, never live listener output. Do not treat a WAV-only pass as validation of the production Debian MP3 encoder or a source connection.
