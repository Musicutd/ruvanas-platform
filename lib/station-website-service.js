...
const websiteSelect = {
  id: true, organisationId: true, productFamily: true, name: true, slug: true, description: true, logoUrl: true,
...
function safeWebsite(station) {
  if (!station || station.productFamily === "CORRECTIONS" || !station.stationWebsiteEnabled || !resolveEntitlements(station.organisation.subscription).serviceEnabled) return null;
  return {
    id: station.id,
    name: station.name,
    slug: station.slug,
