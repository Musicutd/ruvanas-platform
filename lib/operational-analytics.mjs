...
export function operationalAnalyticsCsv(report) {
  ...
  const sections = [[headers, ...rows]];

  const breakdownSections = [
    ["Location performance", report.breakdowns?.locations],
    ["Player performance", report.breakdowns?.players],
    ["Station performance", report.breakdowns?.stations]
  ];
  for (const [title, items] of breakdownSections) {
    if (!items?.length) continue;
    sections.push([
      [[title]],
      [["Name", "Confirmed complete", "Failed or interrupted", "Completion rate"]],
      items.map((item) => [
        item.name,
        item.completed,
        item.exceptions ?? "",
        item.confirmationRate == null ? "" : `${Math.round(item.confirmationRate * 100)}%`
      ])
    ].flat());
  }
