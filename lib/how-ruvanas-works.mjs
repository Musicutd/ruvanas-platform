export const ruvanasProductGuides = Object.freeze([
  ...
  {
    id: "organisations", tabLabel: "Organisations", tabDescription: "Media, events and branches", eyebrow: "RUVANAS ORGANISATIONS",
    title: "Run your organisation’s media without turning Ruvanas into an admin system",
    introduction: "Ruvanas Organisations is a subscriber-operated media, broadcasting and digital communications platform for NGOs, charities, clubs, associations, community groups, civic organisations, federations and networks.",
    outcome: "Governed channels, announcements, live events, podcasts, sponsors and displays, with the subscriber responsible for its service and content.",
    startHref: "/dashboard/organisations", startLabel: "Open Ruvanas Organisations",
    chapters: [
      { title: "1. Choose a template and create branches", summary: "Use familiar language while keeping one product authority.", detail: "Templates adapt labels such as branch, chapter, club location or venue. They never change entitlements and do not add CRM, membership, donor, HR, accounting, election or voter records.", steps: ["Choose the closest organisation template.", "Confirm locale and IANA timezone.", "Create branches or venues in Locations.", "For Network and Enterprise, delegate only the required location scope."], links: [{ href: "/dashboard/organisations/workspace", label: "Organisations workspace" }, { href: "/dashboard/locations", label: "Branches & venues" }] },
      { title: "2. Create channels and continuous programming", summary: "Keep every authorised channel ready between scheduled content.", detail: "Organisation channels reuse the shared scheduler, Smart Playlists and continuous AutoDJ. Selection remains server-side and uses current review, territory, dates, plan catalogue level and ORGANISATIONS_RADIO permission.", steps: ["Create the subscriber-operated channel.", "Prepare eligible owned or catalogue audio.", "Build a music mode and weekly schedule.", "Publish and verify an AutoDJ fallback."], links: [{ href: "/dashboard/organisations/setup", label: "Channel setup" }, { href: "/dashboard/programming", label: "Programming & AutoDJ" }] },
      { title: "3. Publish governed announcements", summary: "Choose every destination explicitly.", detail: "Announcements move from draft to approval to publication. Radio, podcast, display, web-player and mobile-player surfaces are separate choices; Ruvanas never silently publishes to all of them.", steps: ["Write the announcement.", "Choose only intended surfaces and targets.", "Have an owner or manager approve it.", "Publish and review the audit evidence."], links: [{ href: "/dashboard/organisations/workspace?tab=announcements", label: "Announcements" }] },
      { title: "4. Operate Event Mode with fallback", summary: "Plan pre-event, live and post-event states.", detail: "Event Mode records its window, channel and authorised AutoDJ fallback. Going live is a deliberate transition. Ending the event returns control to the prepared fallback rather than leaving the channel without programming.", steps: ["Set the event window and timezone.", "Choose the organisation channel.", "Choose and verify an AutoDJ fallback.", "Mark ready, go live, then end the event cleanly."], links: [{ href: "/dashboard/organisations/workspace?tab=events", label: "Events & live" }] },
      { title: "5. Produce podcasts, campaigns and displays", summary: "Reuse the shared production and delivery core.", detail: "Studio outputs can be handed to announcement, podcast or event workflows. Sponsor profiles add disclosure context to the existing campaign system. Delivery evidence confirms playout; it does not claim advertising impressions.", steps: ["Create and approve media in Studio.", "Send it to the intended Organisations workflow.", "Publish podcasts, campaigns or displays deliberately.", "Review proof-of-play and operational analytics."], links: [{ href: "/dashboard/studio", label: "Ruvanas Studio" }, { href: "/dashboard/podcasts?product=ORGANISATIONS", label: "Podcasts" }, { href: "/dashboard/digital-signage", label: "Digital displays" }] },
      { title: "6. Review rights, isolation and evidence", summary: "Operate within plan, licence and tenant boundaries.", detail: "Organisation-owned media is independent of catalogue access. Licensed catalogue selection remains plan-tier and territory controlled. Every workspace query derives the active organisation server-side and audit records avoid sensitive content.", steps: ["Confirm product and catalogue entitlements.", "Review rights permission and territory.", "Verify branch and team scopes.", "Use Product QA and beta evidence before release."], links: [{ href: "/dashboard/media", label: "Media library" }, { href: "/dashboard/analytics", label: "Analytics & evidence" }] }
    ]
  }
]);

const GUIDE_ID_BY_PRODUCT = Object.freeze({
  RETAIL: "retail",
  SCHOOL: "school",
  ONLINE: "radio",
  HEALTH: "health",
...
export function guideIdsForEntitlements(entitlements = {}) {
  const primary = GUIDE_ID_BY_PRODUCT[String(entitlements.planProductFamily || "").toUpperCase()];
  if (primary) return [primary];

  return [
    entitlements.retailRadioEnabled ? "retail" : null,
    entitlements.schoolRadioEnabled ? "school" : null,
    entitlements.onlineRadioEnabled ? "radio" : null,
    entitlements.healthRadioEnabled ? "health" : null,
    entitlements.faithRadioEnabled ? "faith" : null,
    entitlements.organisationsEnabled ? "organisations" : null
  ].filter(Boolean);
}
