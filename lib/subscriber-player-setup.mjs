...
export function subscriberPlayerAllowance(subscription, instant = new Date()) {
  ...
  const physicalProduct = ["retailRadioEnabled", "schoolRadioEnabled", "healthRadioEnabled", "faithRadioEnabled", "organisationsEnabled"]
    .some((capability) => entitlements[capability]);
  return { enabled: entitlements.serviceEnabled && physicalProduct, limit: physicalProduct ? entitlements.streamLimit : 0, legacy: false, physicalProduct };
}
...
export async function createSubscriberPlayer(database, {
  ...
  return runSerializableTransaction(database, async (tx) => {
    ...
    if (!allowance.enabled || allowance.limit < 1) {
      return { ok: false, status: 403, error: "Area-player setup is unavailable for this product or subscription." };
    }
