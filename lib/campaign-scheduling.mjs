...
export function expandCampaignTargets({ targets, brands = [], groups = [], locations = [], zones = [], stations = [], channels = [] }) {
  ...
  for (const target of targets) {
    ...
    if (target.targetType === "STATION") {
      const station = stationById.get(target.targetId);
      if (!station) throw new Error("A selected station does not belong to this organisation.");
      (station.channelIds || []).forEach((channelId) => {
        (channelById.get(channelId)?.zoneIds || []).forEach((zoneId) => selectedZoneIds.add(zoneId));
      });
    }
    if (target.targetType === "CHANNEL") {
      const channel = channelById.get(target.targetId);
      if (!channel) throw new Error("A selected channel does not belong to this organisation.");
      (channel.zoneIds || []).forEach((zoneId) => selectedZoneIds.add(zoneId));
    }
  }
...
export function campaignTargetCreateData(target) {
  return {
    ...
    stationId: target.targetType === "STATION" ? target.targetId : null,
