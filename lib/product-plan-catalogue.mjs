export const RUVANAS_PRODUCTS = Object.freeze(["RETAIL", "SCHOOL", "ONLINE", "HEALTH", "FAITH", "ORGANISATIONS"]);
...
export function validatePublicPlanCatalogue(plans = PUBLIC_PLAN_CATALOGUE) {
  if (!Array.isArray(plans) || plans.length !== 30) throw new Error("The public plan catalogue must contain exactly thirty plans.");
  ...
  for (const plan of plans) {
    ...
    const enabledProducts = [plan.retailRadioEnabled, plan.schoolRadioEnabled, plan.onlineRadioEnabled, plan.healthRadioEnabled, plan.faithRadioEnabled, plan.organisationsEnabled].filter(Boolean).length;
