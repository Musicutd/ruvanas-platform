...
export async function transitionPlayerLifecycle(prismaClient, {
  ...
  const current = await prismaClient.player.findFirst({
    where: { id: playerId, ...(organisationId ? { organisationId } : {}) },
    include: { replacementPlayer: true }
  });
  ...
  return prismaClient.$transaction(async (tx) => {
    const disabled = await tx.player.updateMany({
      where: { id: current.id, organisationId: current.organisationId, status: { not: "DISABLED" } },
      data: {
        status: "DISABLED",
    ...
    await tx.playerListenerLease.deleteMany({ where: { playerId: current.id, organisationId: current.organisationId } });
