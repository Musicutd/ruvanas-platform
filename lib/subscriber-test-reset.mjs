...
function organisationScopedResetModels() {
  ...
  const allModelsByName = new Map(allModels.map((model) => [model.name, model]));
  ...
  // Some tenant-owned records do not carry organisationId themselves. They
  // are removed by a cascading relation from an organisation-scoped owner
  // (for example PromoVersion -> PromoAsset and AudioClip -> AudioProject).
  // Lift Restrict dependencies from those records to their scoped owners so
  // the owner is cleared before the record it ultimately depends on.
  const cascadeOwnersByModel = new Map(allModels.map((model) => [
    model.name,
    new Set(modelNames.has(model.name) ? [model.name] : [])
  ]));
  let changed = true;
  while (changed) {
    changed = false;
    for (const model of allModels) {
      ...
      const owners = cascadeOwnersByModel.get(model.name);
      for (const field of model.fields) {
        if (
          ...
          field.type === model.name ||
          !allModelsByName.has(field.type)
        ) continue;
        for (const owner of cascadeOwnersByModel.get(field.type)) {
          if (!owners.has(owner)) {
            owners.add(owner);
            changed = true;
          }
  ...
  for (const model of allModels) {
    const consumerOwners = cascadeOwnersByModel.get(model.name);
    if (!consumerOwners.size) continue;
    for (const field of model.fields) {
      if (
        field.kind !== "object" ||
        !field.relationFromFields?.length ||
      ...
      for (const consumerOwner of consumerOwners) {
        for (const dependencyOwner of cascadeOwnersByModel.get(field.type)) {
          if (consumerOwner !== dependencyOwner) {
            parentsByModel.get(consumerOwner).add(dependencyOwner);
          }
        }
      }
    }
