...
import { catalogueLevelForOrganisation } from "@/lib/catalogue-audience-service";
...
export async function radioClockSources(organisationId, instant = new Date()) {
  const [musicModes, tracks, promos, voiceTracks, rundowns, licensedCatalogueLevel] = await Promise.all([
    ...
    prisma.track.findMany({
      ...
      include: cataloguePlaybackTrackInclude,
    ...
    catalogueLevelForOrganisation(prisma, organisationId)
  ]);
  return {
    musicModes: musicModes.filter((mode) => musicModeIsPlayable(mode, instant, { organisationId, requiredUse: "ONLINE_RADIO", licensedCatalogueLevel })).map((mode) => ({ id: mode.id, name: mode.name })),
    tracks: tracks.filter((track) => musicTrackEligibility(track, { organisationId, requiredUse: "ONLINE_RADIO", licensedCatalogueLevel, instant }).playable).map((track) => ({ id: track.id, name: `${track.artist} — ${track.title}`, durationSeconds: track.mediaAsset.durationSeconds })),
...
export async function validateRadioClockSources(client, organisationId, items, { requireReady = false, instant = new Date() } = {}) {
  ...
  const [musicModes, tracks, promos, voiceTracks, rundowns, licensedCatalogueLevel] = await Promise.all([
    ...
    client.voiceTrackSegue.findMany({ where: { id: { in: ids("VOICE_TRACK") }, organisationId }, include: { audioRender: { select: { status: true } }, voicePromoVersion: { include: { mediaAsset: { select: mediaAssetSelect } } }, outgoingTrack: { include: cataloguePlaybackTrackInclude }, incomingTrack: { include: cataloguePlaybackTrackInclude } } }),
    client.schoolRundown.findMany({ where: { id: { in: ids("SHOW_RUNDOWN") }, organisationId }, include: { _count: { select: { items: true } } } }),
    catalogueLevelForOrganisation(client, organisationId)
  ]);
  const found = {
  ...
  for (const item of items) {
    ...
    if (item.type === "MUSIC_MODE" && !musicModeIsPlayable(source, instant, { organisationId, requiredUse: "ONLINE_RADIO", licensedCatalogueLevel })) throw new Error(`${item.label} needs an active music mode with playable tracks.`);
    if (item.type === "MUSIC_TRACK" && !musicTrackEligibility(source, { organisationId, requiredUse: "ONLINE_RADIO", licensedCatalogueLevel, instant }).playable) throw new Error(`${item.label} needs a ready track cleared for Online Radio.`);
    ...
    if (item.type === "VOICE_TRACK" && (source.status !== "APPROVED" || source.audioRender.status !== "SUCCEEDED" || source.voicePromoVersion.status !== "APPROVED" || source.voicePromoVersion.qcStatus !== "PASSED" || source.voicePromoVersion.mediaAsset.status !== "READY" || !musicTrackEligibility(source.outgoingTrack, { organisationId, requiredUse: "ONLINE_RADIO", licensedCatalogueLevel, instant }).playable || !musicTrackEligibility(source.incomingTrack, { organisationId, requiredUse: "ONLINE_RADIO", licensedCatalogueLevel, instant }).playable)) throw new Error(`${item.label} needs a current approved voice-track segue.`);
