...
import { buildAccountRecoveryEmail } from "./account-recovery.mjs";
import { notificationEmailConfig } from "./notification-email.mjs";
...
async function sendWithProvider({ provider, recovery, fetchImpl, dnsLookup }) {
  const message = buildAccountRecoveryEmail({ ...recovery, from: provider.from });
  ...
  try {
    response = await fetchImpl(provider.endpoint, {
      ...
      headers: {
        ...
        "user-agent": "Ruvanas-Account-Recovery/1.0",
...
export async function sendAccountRecoveryEmail({ recovery, fetchImpl = fetch, dnsLookup = dns.lookup, env = process.env }) {
  const config = notificationEmailConfig(env);
  if (!config.configured) return { configured: false, skipped: true };
  try {
    const message = await sendWithProvider({ provider: config, recovery, fetchImpl, dnsLookup });
    ...
    const message = await sendWithProvider({ provider: config.failover, recovery, fetchImpl, dnsLookup });
