...
export function correctionsRenderEvidence(render) {
  ...
  return renderFingerprint(render);
}
...
// Contributor submission begins Corrections Guard review. It deliberately does
// not call assertStudioRenderReady, which remains mandatory for every ordinary
// Studio product handoff and legacy staff submission.
export function correctionsContributorRenderEvidence(render, { organisationId, projectId, versionId } = {}) {
  if (!render || render.status !== "SUCCEEDED" || render.organisationId !== organisationId || render.projectId !== projectId ||
      render.versionId !== versionId || !render.completedAt || render.resultJson?.immutableSource !== true ||
      render.resultJson?.studioPreview || !render.outputMediaAsset || !render.outputPromoVersion) {
    throw new Error("Choose a completed, version-pinned Studio output from this supervised project.");
  }
  if (render.outputMediaAsset.status !== "READY" || render.outputPromoVersion.status !== "IN_REVIEW" ||
      render.outputPromoVersion.sourceReference !== `audio-render:${render.id}`) {
    throw new Error("This Studio output is not pending Corrections review.");
  }
  return renderFingerprint(render);
}

export function correctionsSubmissionEvidence(render, submission) {
  return submission?.evidenceSnapshot?.sourceKind === "SUPERVISED_STUDIO_PENDING_REVIEW"
    ? correctionsContributorRenderEvidence(render, {
      organisationId: submission.organisationId,
      projectId: submission.studioProjectId,
      versionId: submission.studioVersionId
    })
...
export function correctionsSchedulingGate({ programme, submission, reviews = [], organisationPolicy, facilityPolicy, render } = {}) {
  ...
  try {
    if (render?.organisationId !== submission.organisationId || render.id !== submission.renderId || correctionsSubmissionEvidence(render, submission).fingerprint !== submission.sourceFingerprint) return { allowed: false, reason: "SOURCE_CHANGED" };
