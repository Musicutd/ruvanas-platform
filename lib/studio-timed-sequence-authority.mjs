...
const CONTINUATION_MAX_SNAPSHOT_AGE_MS = 1_000;
...
function continuationDenied(reason) {
  return { consistent: false, reason, sourceCommandAllowed: false, listenerVerified: false };
}

// A read-only in-track diagnostic for a future output adapter. It must be
// supplied by one fresh rights/programming/lease snapshot at the time of the
// check. A passing result cannot keep an encoder running: no polling loop,
// source switch, takedown preemption or listener probe is attached here.
export function inspectTimedSequenceContinuation({
  sequence, authority, streamConfig, expectedVersion, expectedTrackId,
  position, workerOwner, instant = new Date()
}) {
  const now = date(instant);
  ...
  if (!now || !start || !end || !capturedAt || end <= start ||
      ...
      !Number.isInteger(position) || position < 0 || position >= sequence.items.length ||
      !Number.isInteger(expectedVersion) || expectedVersion < 1 || !expectedTrackId || !workerOwner) {
    return continuationDenied("SEQUENCE_CONTINUATION_PLAN_INVALID");
  }
  const item = sequence.items[position];
  if (sequence.version !== expectedVersion || item?.position !== position || item.trackId !== expectedTrackId ||
      !Number.isInteger(item.startOffsetSeconds) || !Number.isInteger(item.endOffsetSeconds) ||
      item.startOffsetSeconds < 0 || item.endOffsetSeconds <= item.startOffsetSeconds ||
      start.getTime() + item.endOffsetSeconds * 1000 > end.getTime()) {
    return continuationDenied("SEQUENCE_CONTINUATION_FROZEN_ITEM_MISMATCH");
  }
  const elapsed = now.getTime() - start.getTime();
  if (elapsed < item.startOffsetSeconds * 1000 || elapsed >= item.endOffsetSeconds * 1000 || now >= end) {
    return continuationDenied("SEQUENCE_CONTINUATION_ITEM_NOT_CURRENT");
  }
  if (capturedAt > now || now - capturedAt >= CONTINUATION_MAX_SNAPSHOT_AGE_MS) {
    return continuationDenied("SEQUENCE_CONTINUATION_PLAN_STALE");
  }
  const authorityCapturedAt = date(authority?.capturedAt);
  const coversUntil = date(authority?.coversUntil);
  if (authority?.complete !== true || authority.organisationId !== sequence.organisationId ||
      authority.channelId !== sequence.channelId || !authorityCapturedAt || authorityCapturedAt > now ||
      now - authorityCapturedAt >= CONTINUATION_MAX_SNAPSHOT_AGE_MS ||
      !coversUntil || coversUntil < end || !Array.isArray(authority.candidates) ||
      !Array.isArray(authority.requiredInsertions)) {
    return continuationDenied("SEQUENCE_CONTINUATION_AUTHORITY_INVALID");
  }
  if (authority.requiredInsertions.length) return continuationDenied("SEQUENCE_CONTINUATION_INSERTION_NOT_ARBITRATED");
  if (authority.candidates.length !== 1) return continuationDenied("SEQUENCE_CONTINUATION_COMPETING_SOURCE_NOT_ARBITRATED");
  const candidate = authority.candidates[0];
  const candidateStart = date(candidate?.validFrom);
  const candidateEnd = date(candidate?.validUntil);
  if (candidate?.sourceType !== "PROGRAMME_SCHEDULE" || candidate.sourceId !== sequence.programmeItemId ||
      ...
      candidate.available !== true || !candidateStart || candidateStart.getTime() !== start.getTime() ||
      !candidateEnd || candidateEnd < end) {
    return continuationDenied("SEQUENCE_CONTINUATION_PROGRAMME_MISMATCH");
  }
  const leaseUntil = date(streamConfig?.encoderLeaseUntil);
  if (streamConfig?.outboundAutoDjEnabled !== true || streamConfig.encoderLeaseOwner !== workerOwner ||
      !leaseUntil || leaseUntil.getTime() <= now.getTime() + CONTINUATION_MAX_SNAPSHOT_AGE_MS) {
    return continuationDenied("SEQUENCE_CONTINUATION_ENCODER_LEASE_UNAVAILABLE");
  }
  return { consistent: true, reason: "SEQUENCE_CONTINUATION_CONSISTENT_NOT_ON_AIR",
    sourceCommandAllowed: false, listenerVerified: false };
}
