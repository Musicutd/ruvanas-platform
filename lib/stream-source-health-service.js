...
import { notificationSeverityForIncident } from "./job-notification.mjs";
...
export async function probeStationStream(prismaClient, config, { now = new Date(), fetchImpl, lookupImpl } = {}) {
  ...
  return prismaClient.$transaction(async (tx) => {
    ...
    await enqueueNotificationEvent(tx, {
      organisationId: config.station.organisationId,
      type: "STREAM_ERROR",
      severity: notificationSeverityForIncident(severity),
      title: "Stream source unhealthy",
      message: `${config.station.name} failed repeated public stream health probes.`,
      entityType: "StationStreamHealthIncident",
      entityId: opened.id,
      metadata: { stationId: config.station.id, providerKey: config.providerKey, errorCode: result.errorCode },
      dedupeKey: `stream-error:${opened.id}`,
      correlationId: `stream-health:${opened.id}`,
      occurredAt: now
    });
