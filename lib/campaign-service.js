...
export async function loadCampaignTopology(organisationId) {
  const [brands, groups, locations, stations, channels] = await Promise.all([
    ...
    prisma.station.findMany({
      ...
      select: { id: true, name: true, channels: { where: { status: "ACTIVE" }, select: { id: true } } },
      orderBy: { name: "asc" }
    }),
    prisma.channel.findMany({
      where: { organisationId, status: "ACTIVE" },
      select: { id: true, name: true, stationId: true, zoneAssignments: { where: { activeTo: null }, select: { zoneId: true } } },
      orderBy: { name: "asc" }
    })
  ]);
