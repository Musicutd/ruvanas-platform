...
export function correctionsStudioSameOrigin(request) {
  const origin = request.headers.get("origin");
  if (!origin) return false;
  const requestUrl = new URL(request.url);
  const host = request.headers.get("host");
  return origin === requestUrl.origin || Boolean(host && origin === `${requestUrl.protocol}//${host}`);
}

export function correctionsReviewVersionRequired(render, session, now = new Date()) {
  return Boolean(render && session && !render.resultJson?.studioPreview && session.status === "ACTIVE" &&
    session.organisationId === render.organisationId && session.projectId === render.projectId &&
    session.supervisorUserId === render.requestedByUserId && session.activatedAt && session.expiresAt &&
    new Date(session.activatedAt) <= new Date(render.version?.createdAt) &&
    new Date(session.createdAt) <= new Date(render.createdAt) && new Date(session.expiresAt) > now);
}
