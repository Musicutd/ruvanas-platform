...
import { catalogueLevelForOrganisation } from "./catalogue-audience-service.js";
import { cataloguePlaybackTrackInclude } from "./catalogue-playback-include.js";
...
const include = {
  ...
  outgoingTrack: { include: cataloguePlaybackTrackInclude },
  incomingTrack: { include: cataloguePlaybackTrackInclude }
};

async function loadSources(client, organisationId, input = {}, now = new Date()) {
  input = input || {};
  const [channel, render, outgoingTrack, incomingTrack, licensedCatalogueLevel] = await Promise.all([
    ...
    client.track.findFirst({ where: { id: input.outgoingTrackId }, include: cataloguePlaybackTrackInclude }),
    client.track.findFirst({ where: { id: input.incomingTrackId }, include: cataloguePlaybackTrackInclude }),
  ...
  for (const [track, label] of [[outgoingTrack, "outgoing"], [incomingTrack, "incoming"]]) {
    if (!track || !musicTrackEligibility(track, { organisationId, requiredUse: "ONLINE_RADIO", licensedCatalogueLevel, instant: now }).playable) throw new Error(`The ${label} track is unavailable or not cleared for Online Radio.`);
  }
  const durations = {
...
export async function voiceTrackSegueCatalogue(organisationId, now = new Date()) {
  const [channels, renders, candidateTracks, segues, licensedCatalogueLevel] = await Promise.all([
    ...
    prisma.track.findMany({ where: { status: "READY", mediaAsset: { status: "READY", mediaType: "MUSIC", OR: [{ libraryType: "RUVANAS_CATALOGUE", organisationId: null }, { libraryType: "ORGANISATION_MUSIC", organisationId }] } }, include: cataloguePlaybackTrackInclude, orderBy: [{ artist: "asc" }, { title: "asc" }], take: 500 }),
  ...
  const tracks = candidateTracks.filter((track) => musicTrackEligibility(track, { organisationId, requiredUse: "ONLINE_RADIO", licensedCatalogueLevel, instant: now }).playable);
  return {
...
export async function changeVoiceTrackSegueStatus({ organisationId, segueId, actorUserId, action, expectedVersion, previewAcknowledged = false }) {
  ...
  if (action === "APPROVE") {
    ...
    const licensedCatalogueLevel = await catalogueLevelForOrganisation(prisma, organisationId);
    for (const [track, label] of [[existing.outgoingTrack, "outgoing"], [existing.incomingTrack, "incoming"]]) {
      if (!musicTrackEligibility(track, { organisationId, requiredUse: "ONLINE_RADIO", licensedCatalogueLevel, instant: new Date() }).playable) throw new Error(`The ${label} track is no longer cleared for Online Radio.`);
    }
  }
