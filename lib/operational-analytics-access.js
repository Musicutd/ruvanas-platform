...
export async function requireOperationalAnalyticsAccess({ exportReport = false } = {}) {
  ...
  if (!context.membership || !ORGANISATION_MEMBER_ROLES.includes(role)) {
    return { ok: false, status: 403, error: "Choose an organisation you belong to before viewing analytics." };
  }
  if (exportReport && !ORGANISATION_MANAGER_ROLES.includes(role)) {
    return { ok: false, status: 403, error: "Only organisation owners and managers can export operational analytics." };
  }
  return {
    ok: true,
    user: context.user,
    ...
    canExport: ORGANISATION_MANAGER_ROLES.includes(role)
  };
}
