...
export async function contextForExternalLive() {
  ...
  if (!resolveEntitlements(context.membership.organisation.subscription).serviceEnabled) {
    return { response: NextResponse.json({ error: "External Live is unavailable while this service is inactive." }, { status: 403 }) };
  }
  return { context };
}
