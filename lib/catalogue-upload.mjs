import { z } from "zod";
import { catalogueTierSchema, catalogueUseSchema } from "./catalogue-audience.mjs";
...
const catalogueMetadataSchema = z.object({
  ...
  mixName: z.string().trim().max(120).optional().nullable(),
  bpm: optionalInteger(20, 300),
  ...
  permittedUses: z.array(catalogueUseSchema).min(1, "Choose at least one Ruvanas service permitted by the licence."),
...
export function parseCatalogueMetadata(input) {
  ...
  return {
    ...
    data: {
      ...
      mixName: parsed.data.mixName || null,
