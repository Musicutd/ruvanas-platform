...
export function reorderStudioConsolePanel(input, sourceId, targetId) {
  const layout = normalizeStudioConsoleLayout(input);
  const group = Object.values(STUDIO_CONSOLE_PANEL_GROUPS).find((panels) => panels.includes(sourceId) && panels.includes(targetId));
  if (!group || sourceId === targetId) return layout;
  const visible = layout.panels.filter((id) => group.includes(id));
  ...
  let index = 0;
  return { ...layout, panels: layout.panels.map((id) => group.includes(id) ? reordered[index++] : id) };
}
...
export function formatStudioLogTime(instant, timezone) {
...
export function studioSpotBoard({ policy = null, orders = [] } = {}) {
  return {
    policyStatus: policy?.status || "NOT_CONFIGURED",
    bookings: orders.map((order) => ({
      id: order.id,
      name: order.name,
      advertiser: order.advertiser?.name || "Unspecified advertiser",
      bookingStatus: order.status,
      campaignName: order.campaign?.name || "No linked campaign",
      campaignStatus: order.campaign?.status || "NOT_LINKED",
      inventoryStatus: order.inventoryPackage?.status || "NOT_CONFIGURED",
      creativeStatus: order.creatives?.find((creative) => creative.promoVersionId === order.campaign?.promoVersionId)?.status || "NOT_APPROVED",
      effectiveFrom: order.campaign?.effectiveFrom || null,
      effectiveTo: order.campaign?.effectiveTo || null
    }))
  };
}

export function studioProgrammeLogEntries(occurrence, radioClock = null) {
  const startsAt = new Date(occurrence.startsAt);
  const occurrenceId = `${occurrence.itemId}:${startsAt.toISOString()}`;
  const clockItems = radioClock?.status === "PUBLISHED" && radioClock.publishedVersion > 0 &&
    radioClock.publishedVersion === radioClock.version && Array.isArray(radioClock.items) ? radioClock.items : [];
  const parent = {
    id: occurrenceId, sourceType: occurrence.sourceType, sourceId: occurrence.sourceId,
    label: occurrence.label, startsAt,
    // A clock's children are the timed audio. Its programme row is a boundary,
    // not a second full-hour play that should push every child into the future.
    durationMs: clockItems.length ? 0 : occurrence.endsAt - startsAt,
    hardEvent: true, programmeOccurrenceId: occurrenceId, programmeBoundary: clockItems.length > 0
  };
  return [parent, ...clockItems.map((item) => ({
    id: `clock:${occurrenceId}:${item.id}`, sourceType: `RADIO_CLOCK_${item.type}`,
    sourceId: item.id, label: item.label,
    startsAt: new Date(startsAt.getTime() + item.offsetSeconds * 1000),
    durationMs: item.durationSeconds * 1000, locked: true,
    programmeOccurrenceId: occurrenceId
  }))];
}

export function studioTimedPlaylistLogEntries(playlist, startsAt, scheduled = []) {
  const version = playlist?.versions?.find((item) => item.version === playlist.publishedVersion && item.publishedAt);
  if (!version) return { scheduled, generated: [] };
  const startMs = new Date(startsAt).getTime();
  if (!Number.isFinite(startMs)) throw new Error("The published timed playlist has no valid local start.");
  const parent = scheduled.find((item) => item.sourceType === "MUSIC_MODE" && item.sourceId === playlist.musicModeId &&
    new Date(item.startsAt).getTime() === startMs);
  return {
    // The published programme is a boundary for its frozen child sequence,
    // not another full-duration play to serialise before every track.
    scheduled: parent ? scheduled.map((item) => item === parent ? { ...item, durationMs: 0, programmeBoundary: true } : item) : scheduled,
    generated: version.items.map((item) => ({
      id: item.id, sourceType: "TIMED_PLAYLIST", sourceId: playlist.id,
      label: `${item.track.artist} — ${item.track.title}`,
      startsAt: new Date(startMs + item.startOffsetSeconds * 1000),
      durationMs: item.durationSeconds * 1000, locked: true,
      programmeOccurrenceId: parent?.programmeOccurrenceId || null
    }))
  };
}
...
export function deriveStudioDailyLog({ scheduled = [], generated = [], campaigns = [], live = [], manual = [], proof = [], dayStart, dayEnd } = {}) {
  ...
  const planned = [...scheduled, ...generated, ...campaigns, ...live, ...manual]
    ...
    .map((item) => ({
      ...
      hardEvent: item.hardEvent === true, rightsReady: item.rightsReady ?? null,
      programmeOccurrenceId: item.programmeOccurrenceId || null, programmeBoundary: item.programmeBoundary === true,
  ...
  for (const item of planned) {
    ...
    // Published clock offsets already account for intentional crossfades.
    // Do not serialise those children a second time in the planning view.
    const estimatedMs = item.hardEvent || item.sourceType.startsWith("RADIO_CLOCK_") || item.sourceType === "TIMED_PLAYLIST" ? plannedMs : Math.max(plannedMs, cursor);
...
export function studioClockBoundaryConflicts(log) {
  const planned = log?.planned || [];
  return planned.filter((entry) => entry.hardEvent).flatMap((boundary) => {
    const at = new Date(boundary.plannedStartAt).getTime();
    return planned.filter((entry) => entry.sourceType.startsWith("RADIO_CLOCK_") &&
      entry.programmeOccurrenceId !== boundary.programmeOccurrenceId &&
      new Date(entry.plannedStartAt).getTime() < at &&
      new Date(entry.plannedStartAt).getTime() + entry.durationMs > at
    ).map((entry) => ({
      clockItemId: entry.id, clockLabel: entry.label,
      boundaryId: boundary.id, boundaryLabel: boundary.label,
      boundaryAt: boundary.plannedStartAt,
      overlapMs: new Date(entry.plannedStartAt).getTime() + entry.durationMs - at
    }));
  });
}
