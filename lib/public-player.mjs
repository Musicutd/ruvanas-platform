...
export async function claimPublicListenerLease(database, { station, channelId, sessionId, instant = new Date(), secret }) {
  ...
  const capability = station.productFamily === "HEALTH" ? "healthRadioEnabled" : station.productFamily === "FAITH" ? "faithRadioEnabled" : "onlineRadioEnabled";
  const productEnabled = Boolean(entitlements[capability]);
  const audienceAllowed = !["HEALTH", "FAITH"].includes(station.productFamily) || station.audiencePolicy === "PUBLIC";
