...
// Token checks already deny expired access. Persist the transition so staff
// can recover saved work and see one durable audit event per expired session.
export async function expireCorrectionsStudioSessions(organisationId, now = new Date()) {
  const expired = await prisma.correctionsStudioSession.findMany({ where: {
    ...(organisationId ? { organisationId } : {}), status: "ACTIVE", expiresAt: { lte: now }
  }, select: { id: true, organisationId: true, facilityId: true, projectId: true } });
  for (const session of expired) await runSerializableTransaction(prisma, async (tx) => {
    const changed = await tx.correctionsStudioSession.updateMany({ where: { id: session.id, status: "ACTIVE", expiresAt: { lte: now } },
      data: { status: "EXPIRED", accessTokenHash: null, completedAt: now } });
    if (changed.count) await tx.auditLog.create({ data: { organisationId: session.organisationId,
      action: "CORRECTIONS_STUDIO_SESSION_EXPIRED", entityType: "CorrectionsStudioSession", entityId: session.id,
      details: { facilityId: session.facilityId, projectId: session.projectId, expiredAt: now.toISOString() } } });
  });
...
export async function listCorrectionsStudio(access) {
  ...
  await expireCorrectionsStudioSessions(organisationId);
...
export async function activateCorrectionsStudioSession(access, sessionId, minutes) {
  ...
  await expireCorrectionsStudioSessions(access.organisationId);
  return runSerializableTransaction(prisma, async (tx) => {
...
export async function findCorrectionsContributorSession(token, { allowSubmitted = false } = {}) {
  ...
  if (session.status === "ACTIVE" && session.expiresAt && new Date(session.expiresAt) <= new Date()) {
    await expireCorrectionsStudioSessions(session.organisationId);
    return null;
  }
