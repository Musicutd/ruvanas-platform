...
import { catalogueTerritoryAllows } from "./catalogue-territories.mjs";
...
export function musicTrackEligibility(track, {
  ...
  if (asset.libraryType === "RUVANAS_CATALOGUE") {
    ...
    if (asset.licensedCatalogue === true) {
      ...
      if (track.minimumCatalogueLevel && track.minimumCatalogueLevel !== "NONE" && !catalogueLevelAllows(licensedCatalogueLevel, track.minimumCatalogueLevel)) return { playable: false, reason: "CATALOGUE_TIER_REQUIRED" };
      ...
      if (requiredUse && !track.permittedUses?.includes(requiredUse)) return { playable: false, reason: "USE_NOT_PERMITTED" };
    ...
    // Preserve legacy global releases, but honour product restrictions recorded
    // on newer Super Admin uploads instead of leaking them into other pillars.
    if (requiredUse && track.permittedUses?.length && !track.permittedUses.includes(requiredUse)) {
      return { playable: false, reason: "USE_NOT_PERMITTED" };
    }
