...
import { catalogueTerritoryAllows, parseCatalogueTerritories } from "./catalogue-territories.mjs";
import { catalogueLevelAllows, genreCodesForTrack, licensedGenresForLevel, normaliseGenreCode } from "./autodj-genre-entitlements.mjs";
import { effectiveCatalogueLevel } from "./catalogue-audience.mjs";
...
const LEGACY_SHARED_MUSIC_RIGHTS_USES = Object.freeze([
  "RETAIL_RADIO",
  "SCHOOL_RADIO",
  "ONLINE_RADIO"
]);
...
function territoryAllowed(permittedTerritories, territory) {
  ...
  return catalogueTerritoryAllows(permittedTerritories, territory);
}
...
export function musicTrackEligibility(track, {
  ...
  requiredUse = null,
  territory = null,
  licensedCatalogueLevel = null,
  selectedGenreCodes = null,
  configuredGenres = [],
  instant = new Date()
} = {}) {
  ...
  if (asset.libraryType === "RUVANAS_CATALOGUE") {
    ...
    if (!parseCatalogueTerritories(track.permittedTerritories).ok) return { playable: false, reason: "TERRITORY_REQUIRED" };
    if (!licensedCatalogueLevel || !catalogueLevelAllows(licensedCatalogueLevel, "FOCUSED")) return { playable: false, reason: "CATALOGUE_PLAN_REQUIRED" };
    if (!catalogueLevelAllows(licensedCatalogueLevel, effectiveCatalogueLevel(track.minimumCatalogueLevel))) return { playable: false, reason: "CATALOGUE_TIER_REQUIRED" };
    if (!requiredUse) return { playable: false, reason: "PRODUCT_CONTEXT_REQUIRED" };
    if (!track.permittedUses?.includes(requiredUse)) return { playable: false, reason: "USE_NOT_PERMITTED" };
    if (asset.licensedCatalogue === true) {
      if (track.catalogueProvider === "PROMO_ONLY" && (!track.distributorItems?.length || track.distributorItems.some((item) => item.status !== "ACTIVE" || item.autoDjReady !== true || item.canonicalGenre?.active !== true || item.canonicalGenre?.providerReviewStatus !== "APPROVED"))) return { playable: false, reason: "PROVIDER_TRACK_INACTIVE" };
      if (!territory && !catalogueTerritoryAllows(track.permittedTerritories, null)) return { playable: false, reason: "TERRITORY_REQUIRED" };
      if (track.distributorItems?.length && track.distributorItems.some((item) => item.status !== "ACTIVE" || item.autoDjReady === false)) return { playable: false, reason: "PROVIDER_TRACK_INACTIVE" };
      const allowedGenres = new Set(licensedGenresForLevel(licensedCatalogueLevel, configuredGenres).map((genre) => genre.code));
      for (const item of track.distributorItems || []) {
        const genre = item.canonicalGenre;
        if (genre?.active && genre.providerReviewStatus === "APPROVED" && catalogueLevelAllows(licensedCatalogueLevel, genre.minimumCatalogueLevel)) {
          allowedGenres.add(normaliseGenreCode(genre.slug || genre.name));
        }
      }
      const trackGenres = genreCodesForTrack(track);
      if (!trackGenres.some((genre) => allowedGenres.has(genre))) return { playable: false, reason: "CATALOGUE_GENRE_LOCKED" };
      if (Array.isArray(selectedGenreCodes) && selectedGenreCodes.length && !trackGenres.some((genre) => selectedGenreCodes.includes(genre))) return { playable: false, reason: "GENRE_NOT_SELECTED" };
      if (track.rightsReviewStatus !== "APPROVED") return { playable: false, reason: "RIGHTS_NOT_APPROVED" };
      if (!territoryAllowed(track.permittedTerritories, territory)) return { playable: false, reason: "RIGHTS_NOT_APPROVED" };
      return { playable: true, reason: "LICENSED_CATALOGUE" };
    }
    // Core releases still have their own genre workflow, but never bypass
    // the Super Admin's central pillar and tier controls.
    if (track.permittedTerritories && !territory && !catalogueTerritoryAllows(track.permittedTerritories, null)) {
      return { playable: false, reason: "TERRITORY_REQUIRED" };
    }
    if (!territoryAllowed(track.permittedTerritories, territory)) {
  ...
  } else if (!LEGACY_SHARED_MUSIC_RIGHTS_USES.every((use) => uses.includes(use))) {
    // Preserve the established three-product shared resolver for callers that
    // pre-date Health and Faith. New product entry points always supply the
    // required use explicitly and therefore cannot inherit legacy clearance.
