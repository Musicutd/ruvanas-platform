...
export async function deliverProtectedPodcastAudio(request, asset, { onDelivery = null, cacheControl = "public, max-age=300" } = {}) {
  const total = Number(asset?.sizeBytes);
  if (!asset || !Number.isFinite(total) || total <= 0) {
    return NextResponse.json({ error: "The published audio has an invalid size." }, { status: 500 });
  }
  ...
  if (onDelivery) await onDelivery({ bytesOffered: length, rangeRequest: Boolean(range) });
