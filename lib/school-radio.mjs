...
function schoolScheduleItemId(playerId, slot) {
...
export function compileSchoolRadioPlayout({
  ...
  for (const slot of slots) {
    ...
    if (
      ...
      announcement.organisationId !== player.organisationId ||
      !targetMatches(slot, player) ||
      plannedStart < bucketStart ||
      plannedStart >= horizonEnd ||
    ...
    insertions.push({
      scheduleItemId: schoolScheduleItemId(player.id, slot),
      schoolBroadcastSlotId: slot.id,
      announcementId: announcement.id,
      announcementTitle: announcement.title,
      publicationRevision: slot.revision,
      sourceRevision: `${slot.id}:${slot.revision}:${announcement.policyVersion}`,
