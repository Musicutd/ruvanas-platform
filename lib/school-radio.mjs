...
import { importedExchangeIsPlayable } from "./school-network-exchange.mjs";
...
function mediaAllowedForPlayer(mediaAsset, player, { allowForeign = false } = {}) {
...
export function compileSchoolRadioPlayout({
  ...
  for (const slot of slots) {
    ...
    if (announcement) {
      ...
      const exchangeRequest = announcement.sourceExchangeRequest;
      const sharedAudioAllowed = Boolean(
        exchangeRequest &&
        importedExchangeIsPlayable(announcement, player.organisationId) &&
        exchangeRequest.offer.sourceOrganisationId === mediaAsset?.organisationId &&
        exchangeRequest.offer.approvedPromoVersionId === promoVersion?.id
      );
      if (
        ...
        promoVersion?.promoAsset?.status !== "ACTIVE" || !mediaAllowedForPlayer(mediaAsset, player, { allowForeign: sharedAudioAllowed })
      ) continue;
      insertions.push({
