export const PODCAST_PRODUCTS = Object.freeze({
  ...
  HEALTH_RADIO: "HEALTH_RADIO",
  FAITH_RADIO: "FAITH_RADIO",
  ORGANISATIONS_RADIO: "ORGANISATIONS_RADIO"
});
...
export function validatePodcastPublication({
  ...
  if (!series || !Object.values(PODCAST_PRODUCTS).includes(series.product)) {
    throw new Error("Choose a supported Ruvanas podcast series.");
  }
...
export function validateOnlinePodcastPublication(input = {}) {
  if (input?.series?.product !== PODCAST_PRODUCTS.ONLINE_RADIO) {
    throw new Error("Choose an Online Radio podcast series.");
  }
  return validatePodcastPublication(input);
}
