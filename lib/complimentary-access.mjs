...
export function complimentaryPlanSnapshot(plan) {
  ...
  return {
    ...
    complimentaryPlanTierNumber: plan.tierNumber,
    complimentaryPlanProductFamily: plan.productFamily,
    complimentaryStudioExternalDestinationLimit: plan.studioExternalDestinationLimit ?? null,
    ...
    complimentaryCorrectionsRadioEnabled: plan.correctionsRadioEnabled,
...
export function clearComplimentaryAccess() {
  return {
    ...
    complimentaryPlanTierNumber: null,
    complimentaryPlanProductFamily: null,
    complimentaryStudioExternalDestinationLimit: null,
    ...
    complimentaryCorrectionsRadioEnabled: null,
...
export function resolveComplimentaryPlan(subscription) {
  ...
  // Recover older grants only from a plan with the exact granted code.
  const matchingPlan = subscription.plan?.code === subscription.complimentaryPlanCode
    ? subscription.plan
    : findPublicPlan(subscription.complimentaryPlanCode);

  return {
    active: true,
    name: subscription.complimentaryPlanName,
    code: subscription.complimentaryPlanCode,
    tierNumber: subscription.complimentaryPlanTierNumber ?? matchingPlan?.tierNumber ?? null,
    productFamily: subscription.complimentaryPlanProductFamily ?? matchingPlan?.productFamily ?? null,
    studioExternalDestinationLimit: subscription.complimentaryStudioExternalDestinationLimit ?? matchingPlan?.studioExternalDestinationLimit ?? null,
    ...
    correctionsRadioEnabled: Boolean(subscription.complimentaryCorrectionsRadioEnabled),
...
export function describePlanFeatures(plan) {
  return [
    ...
    plan.correctionsRadioEnabled ? "Ruvanas Inside" : null,
...
export function complimentaryPlanProducts(plan) {
  ...
  if (plan?.correctionsRadioEnabled) products.push("Ruvanas Inside");
  return products;
}
