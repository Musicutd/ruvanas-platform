...
export async function probeStreamEndpoint(value, { timeoutMs = 8_000, fetchImpl = fetch, lookupImpl = dnsLookup, now = () => Date.now() } = {}) {
  const startedAt = now();
  let url;
  try {
  ...
  try {
    const response = await fetchImpl(url, {
      method: "GET",
      headers: { Accept: "audio/*,*/*;q=0.1", Range: "bytes=0-0", "User-Agent": "Ruvanas-Stream-Probe/1.0" },
      redirect: "manual",
      cache: "no-store",
