...
export function publishedGeneratedPlaylist(musicMode) {
  return musicMode?.generatedPlaylists?.find((playlist) =>
    playlist.publishedVersion > 0 && ["PUBLISHED", "DRAFT"].includes(playlist.status)) || null;
}

export function playableMusicModeEntries(musicMode, instant = new Date(), options = {}) {
  const generated = publishedGeneratedPlaylist(musicMode);
