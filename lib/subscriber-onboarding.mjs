...
export function buildSubscriberOnboarding({
  ...
  const steps = orderedSteps([
    ...
    {
      id: "LOCATION",
      label: "Prepare a listening location",
      detail: activeLocationCount > 0
        ? "At least one active location or listening area is available."
        : "Ruvanas prepares the location and listening zone before player enrolment.",
      owner: "Ruvanas setup",
      complete: activeLocationCount > 0,
      href: "/dashboard/players",
      actionLabel: "Check location setup"
    },
    {
      id: "PROGRAMME",
    ...
    {
      id: "PLAYER",
      label: "Prepare your first player",
      detail: configuredPlayerCount > 0
        ? "A secure player has been prepared for this organisation."
        : "Create one enrolled player for the location or listening zone.",
      owner: canManage ? "Your task" : "Owner or manager",
