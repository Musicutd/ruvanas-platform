...
export function correctionsError(error, fallback = "This Corrections action could not be completed.") {
  ...
  if (error?.constructor === Error && typeof error.message === "string" && error.message.length <= 220) {
    const status = [400, 401, 403, 404, 409, 429].includes(error.status) ? error.status : /permission|not active|not available/i.test(error.message) ? 403 : /already|current|before|Wait|review|changed/i.test(error.message) ? 409 : 400;
