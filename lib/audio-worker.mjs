...
export function buildMultitrackRenderGraph(value, options = {}) {
  ...
  const master = options.processingProfile ? buildBroadcastProcessingFilters(options.processingProfile) : [];
  if (!options.processingProfile && state.master.normalize) master.push(`loudnorm=I=${state.master.targetLufs}:TP=-1.5:LRA=11`);
  if (!options.processingProfile && state.master.limiter) master.push("alimiter=limit=0.95");
