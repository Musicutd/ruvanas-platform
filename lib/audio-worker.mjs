...
import { buildBroadcastProcessingFilters } from "./broadcast-audio-processing.mjs";
import { buildVoiceCleanupFilters } from "./voice-cleanup.mjs";
import { applyStudioEffectPreset, buildStudioEffectsFilters, buildStudioMasteringFilters, normalizeStudioMastering } from "./studio-effects-mastering.mjs";
...
export function buildRenderGraph(clips, settings = {}) {
  ...
  const finalChain = [];
  if (settings.voiceCleanup) finalChain.push(...buildVoiceCleanupFilters(settings.voiceCleanup, { bypass: settings.voiceCleanupBypass === true }));
  else if (settings.noiseCleanup && settings.voiceCleanupBypass !== true) finalChain.push("afftdn=nf=-25");
  finalChain.push(...buildStudioEffectsFilters(settings.effects));
  if (settings.processingProfile) finalChain.push(...buildBroadcastProcessingFilters(settings.processingProfile));
  else if (settings.mastering) finalChain.push(...buildStudioMasteringFilters(settings.mastering));
  else if (settings.normalize !== false) finalChain.push(`loudnorm=I=${Number(settings.targetLufs || -16)}:TP=-1.5:LRA=11`);
...
function presetFilters(preset) {
  const mapped = preset === "SPEECH_CLEANUP" ? "CLEAN_INTERVIEW" : preset === "RADIO_VOICE" ? "BROADCAST_VOICE" : preset;
  return buildStudioEffectsFilters(applyStudioEffectPreset(mapped));
}
...
export function buildMultitrackRenderGraph(value, options = {}) {
  ...
  const master = [
    ...buildStudioEffectsFilters(state.master.effects),
    ...(options.processingProfile ? buildBroadcastProcessingFilters(options.processingProfile) : buildStudioMasteringFilters(normalizeStudioMastering(state.master, state.master)))
  ];
