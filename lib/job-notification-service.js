...
export async function deliverNotificationEvent(prismaClient, job, now = new Date(), {
  ...
  let memberships = await prismaClient.organisationMember.findMany({
    where: { organisationId: event.organisationId, role: { not: "STUDENT" } },
    select: { id: true, role: true, userId: true, user: { select: { email: true } } }
  });
  if (event.type === "CORRECTIONS_REVIEW_REQUEST") {
    const facilityId = event.metadata?.facilityId;
    const grants = facilityId ? await prismaClient.correctionsFacilityGrant.findMany({ where: { organisationId: event.organisationId, facilityId, permission: "MANAGER" }, select: { organisationMemberId: true } }) : [];
    const allowed = new Set(grants.map((grant) => grant.organisationMemberId));
    memberships = memberships.filter((member) => member.role === "OWNER" || (member.role === "MANAGER" && allowed.has(member.id)));
  }
  ...
  const webhooksQueued = event.type === "CORRECTIONS_REVIEW_REQUEST" ? 0 : await webhookQueue(prismaClient, {
    organisationId: event.organisationId,
