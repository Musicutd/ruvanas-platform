...
export async function claimPlayerListenerLease(database, {
  ...
  const result = await runSerializableTransaction(database, async (tx) => {
    ...
    const heldLease = await tx.playerListenerLease.findUnique({
      where: { playerId_instanceHash: { playerId: player.id, instanceHash } },
      select: { revokedAt: true, expiresAt: true }
    });
    if (heldLease?.revokedAt && heldLease.expiresAt > instant) {
      return { ok: false, revoked: true, activeCount: null };
    }
    const active = await tx.playerListenerLease.findMany({
      where: { organisationId: player.organisationId, revokedAt: null, expiresAt: { gt: instant } },
    ...
    if (existingIndex >= 0) {
      const firstPlayerLeaseIndex = active.findIndex((lease) => lease.playerId === player.id);
      if (existingIndex !== firstPlayerLeaseIndex) {
        await tx.playerListenerLease.delete({ where: { id: active[existingIndex].id } });
        return { ok: false, deviceInUse: true, activeCount: Math.min(active.length - 1, limit) };
      }
    ...
    if (active.some((lease) => lease.playerId === player.id)) {
      return { ok: false, deviceInUse: true, activeCount: Math.min(active.length, limit) };
    }
  ...
  if (!result.ok) {
    if (result.revoked) {
      return {
        ...
        error: "This player session was stopped from the subscriber dashboard.",
        limit,
        activeCount: null,
    ...
    if (result.deviceInUse) {
      return {
        ok: false,
        status: 409,
        code: "PLAYER_DEVICE_IN_USE",
        error: "This enrolled player is already active on another device.",
        limit,
...
export async function releasePlayerListenerLease(database, { player, instanceId, secret }) {
  ...
  const deleted = await database.playerListenerLease.deleteMany({
    where: { playerId: player.id, organisationId: player.organisationId, instanceHash, revokedAt: null }
  });
...
export async function isPlayerListenerTokenActive(database, { player, token, instant = new Date(), secret }) {
  ...
  return Boolean(lease && !lease.revokedAt && lease.expiresAt > instant);
}
