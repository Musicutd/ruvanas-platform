...
export function eligibleOnlineRadioRotation(station, entitlements, instant = new Date(), configuredGenres = []) {
  ...
  const activeChannels = station.channels?.filter((item) => item.status === "ACTIVE") || [];
  if (!activeChannels.length) return { ready: false, reason: "CHANNEL_NOT_ACTIVE" };
  if (activeChannels.length !== 1) return { ready: false, reason: "CHANNEL_AMBIGUOUS" };
  const channel = activeChannels[0];
  if (channel.stationId !== station.id || channel.organisationId !== station.organisationId) {
    return { ready: false, reason: "CHANNEL_SCOPE_MISMATCH" };
  }
...
export function rotationFingerprint(rotation) {
  ...
  const payload = {
    ...
    bitrateKbps: rotation.bitrateKbps ?? station.streamConfig.bitrateKbps, policyUpdatedAt: policy.updatedAt,
