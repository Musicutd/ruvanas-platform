...
import { assertFutureTimedPublication, assertFutureTimedReplacement, firstInvalidTimedPublicationItem, programmeItemData, replacePublishedAreaSchedule, replacePublishedProgrammeItem, uniquePublishedModeTracks } from "@/lib/generated-playlist-publication.mjs";
...
export async function publishGeneratedPlaylist({ organisationId, playlistId, actorUserId, catalogueLevel, configuredGenres = [] }) {
  ...
  else assertFutureTimedPublication(startsAt);
  return prisma.$transaction(async (tx) => {
    ...
    const invalid = firstInvalidTimedPublicationItem(version.items, { startsAt, organisationId, rightsUse: current.rightsUse, territory: current.territory, catalogueLevel, selectedGenreCodes: current.selectedGenreCodes, sourceScopes: current.sourceScopes, configuredGenres });
    if (invalid) {
      const error = new Error(`A generated track or its planned play window is no longer valid (${invalid.reason}). Regenerate before publishing.`); error.code = "GENERATION_INVALIDATED"; throw error;
    }
