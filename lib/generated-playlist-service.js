...
export async function publishGeneratedPlaylist({ organisationId, playlistId, actorUserId, catalogueLevel, configuredGenres = [] }) {
  ...
  const version = await prisma.generatedPlaylistVersion.findUnique({
    ...
    include: { items: { orderBy: { position: "asc" }, include: { track: { include: cataloguePlaybackTrackInclude } } } }
  });
  ...
  return prisma.$transaction(async (tx) => {
    await assertCorrectionsSchedulingAllowed(tx, { organisationId, ...(current.targetType === "LOCATION" ? { locationId: current.targetId } : current.targetType === "ZONE" ? { zoneId: current.targetId } : { channelId: current.targetId }) });
