...
import { inspectTimedSequenceAuthority, inspectTimedSequenceContinuation, inspectTimedSequenceItemAdmission } from "./studio-timed-sequence-authority.mjs";
...
async function readTimedItemSnapshot(database, { organisationId, channelId, playlistId, capturedAt }) {
...
export async function loadPublishedTimedChannelItemAdmission(database, {
  ...
  try {
    const snapshot = await readTimedItemSnapshot(database, { organisationId, channelId, playlistId, capturedAt });
...
// A separate in-track snapshot for future preemption handling. It uses the
// same transaction as boundary admission, but no live worker calls it and a
// passing result is never an encoder command or independent listener proof.
export async function loadPublishedTimedChannelContinuation(database, {
  organisationId, channelId, playlistId, expectedVersion, expectedTrackId,
  position, workerOwner, clock = () => new Date()
}) {
  ...
  if (!organisationId || !channelId || !playlistId || !workerOwner || !expectedTrackId ||
      !Number.isInteger(expectedVersion) || expectedVersion < 1 ||
      !Number.isInteger(position) || position < 0) return denied("SEQUENCE_CONTINUATION_SCOPE_INVALID");
  const capturedAt = clock();
  if (!(capturedAt instanceof Date) || Number.isNaN(capturedAt.valueOf())) return denied("SEQUENCE_CLOCK_INVALID");
  try {
    const snapshot = await readTimedItemSnapshot(database, { organisationId, channelId, playlistId, capturedAt });
    if (snapshot.reason) return denied(snapshot.reason);
    const checkedAt = clock();
    if (!(checkedAt instanceof Date) || Number.isNaN(checkedAt.valueOf()) || checkedAt < capturedAt ||
        checkedAt.getTime() - capturedAt.getTime() >= 1_000) return denied("SEQUENCE_CONTINUATION_SNAPSHOT_STALE");
    return inspectTimedSequenceContinuation({ ...snapshot, expectedVersion, expectedTrackId, position, workerOwner, instant: checkedAt });
  } catch {
    return denied("SEQUENCE_SNAPSHOT_FAILED");
  }
}
