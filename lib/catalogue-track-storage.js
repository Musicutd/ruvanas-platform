...
export async function storeCatalogueTrack({ buffer, fileName, claimedType = "", metadata, actorUserId, inspected = null, auditContext = null, reviewApproved = true }) {
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      if (metadata.genreIds.length) {
        await tx.mediaAssetGenre.createMany({
          data: metadata.genreIds.map((mediaGenreId, index) => ({ mediaAssetId: mediaAsset.id, mediaGenreId, isPrimary: index === 0 })),
          skipDuplicates: true
        });
      }
