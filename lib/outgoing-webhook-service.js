...
export async function queueOutgoingWebhookEventForConnection(tx, { organisationId, connectionId, eventType, sourceId, version, payload }) {
  const connection = await tx.integrationConnection.findFirst({
    where: {
      ...
      revokedAt: null,
      subscribedEventTypes: { has: eventType }
    },
    select: { id: true }
  });
  if (!connection) throw new Error("The distribution adapter connection is unavailable.");
  const safePayload = publicWebhookPayload(eventType, payload);
  const idempotencyKey = webhookIdempotencyKey(eventType, sourceId, version);
  const result = await tx.outgoingWebhookEvent.createMany({
    data: [{ organisationId, connectionId: connection.id, eventType, idempotencyKey, payload: safePayload }],
    skipDuplicates: true
  });
  return result.count;
}
