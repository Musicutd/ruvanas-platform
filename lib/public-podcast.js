...
import { PODCAST_PRODUCTS, publicPodcastEpisode, validatePodcastPublication } from "@/lib/podcast-core.mjs";
...
async function activeOrganisation(slug) {
  ...
  const entitlements = organisation ? resolveEntitlements(organisation.subscription) : null;
  if (!organisation || !entitlements.serviceEnabled) return null;
  return { ...organisation, entitlements };
}
...
export async function loadPublicPodcastSeries(organisationSlug, feedSlug) {
  ...
  const series = await prisma.schoolPodcastSeries.findFirst({
    where: {
      ...
      product: { in: [PODCAST_PRODUCTS.ONLINE_RADIO, PODCAST_PRODUCTS.HEALTH_RADIO, PODCAST_PRODUCTS.FAITH_RADIO, PODCAST_PRODUCTS.ORGANISATIONS_RADIO] },
      feedSlug,
      rssEnabled: true,
      publicationScope: "PUBLIC"
    },
    include: {
      station: { select: { name: true, slug: true, status: true, stationWebsiteEnabled: true, audiencePolicy: true } },
  ...
  const capability = series.product === PODCAST_PRODUCTS.HEALTH_RADIO
    ? "healthRadioEnabled"
    : series.product === PODCAST_PRODUCTS.FAITH_RADIO
      ? "faithRadioEnabled"
      : series.product === PODCAST_PRODUCTS.ORGANISATIONS_RADIO
        ? "organisationsEnabled"
        : "onlineRadioEnabled";
  if (!organisation.entitlements[capability]) return null;
  if (series.product !== PODCAST_PRODUCTS.ONLINE_RADIO && series.station.audiencePolicy !== "PUBLIC") return null;
  const episodes = [];
  for (const episode of series.episodes) {
    const audio = approvedAsset(episode, organisation.id);
    try {
      validatePodcastPublication({ series, episode, approvedAudio: audio, transcriptStatus: episode.transcript?.status, stationStatus: series.station.status });
