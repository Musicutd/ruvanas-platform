...
export async function loadPublicPodcastSeries(organisationSlug, feedSlug) {
  ...
  const series = await prisma.schoolPodcastSeries.findFirst({
    where: {
      ...
      product: { in: [PODCAST_PRODUCTS.ONLINE_RADIO, PODCAST_PRODUCTS.HEALTH_RADIO, PODCAST_PRODUCTS.FAITH_RADIO] },
      feedSlug,
      rssEnabled: true,
  ...
  const capability = series.product === PODCAST_PRODUCTS.HEALTH_RADIO ? "healthRadioEnabled" : series.product === PODCAST_PRODUCTS.FAITH_RADIO ? "faithRadioEnabled" : "onlineRadioEnabled";
