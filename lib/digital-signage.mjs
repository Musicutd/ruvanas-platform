...
export function normaliseDigitalSignageEnrolmentCode(value) {
  const entered = typeof value === "string" ? value.trim() : "";
  if (!entered) return "";
  const labelledCode = entered.match(/(?:one-time\s+)?enrolment code\s*:\s*([A-Za-z0-9_-]{43})/i);
  return labelledCode?.[1] || entered;
}
...
export function validateDigitalSignageVideo({ buffer, fileName = "", claimedType = "" } = {}) {
  if (!Buffer.isBuffer(buffer) || buffer.length === 0) return { ok: false, error: "Choose a video before uploading." };
  if (buffer.length > MAX_VIDEO_BYTES) return { ok: false, error: "The video exceeds the 80 MB upload limit." };
  const detected = detectedVideo(buffer);
  if (!detected) return { ok: false, error: "Only valid MP4 and WebM videos are accepted." };
  const normalizedType = String(claimedType || "").split(";", 1)[0].toLowerCase();
  if (normalizedType && normalizedType !== detected.mimeType) return { ok: false, error: "The file content does not match its reported video type." };
  const extension = String(fileName).split(".").pop()?.toLowerCase();
  if (extension && extension !== detected.extension) return { ok: false, error: "The file extension does not match the video content." };
  return { ok: true, ...detected, sizeBytes: buffer.length, kind: "VIDEO" };
}
