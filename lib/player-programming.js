...
const playbackModeInclude = {
  generatedPlaylists: { where: { status: { in: ["PUBLISHED", "DRAFT"] }, publishedVersion: { gt: 0 } }, select: { status: true, publishedVersion: true, selectedGenreCodes: true }, orderBy: { publishedAt: "desc" }, take: 1 },
...
export async function resolvePlayerProgramming(player, instant = new Date(), {
  ...
  const [schedules, campaigns, schoolSlots, autoDjPolicy, programmeSchedule, externalLiveSource, liveFailoverPolicy, radioAdvertisingPolicy] = await Promise.all([
  ...
  const legacyResolution = resolveMusicSchedule({
    ...
    musicModeAvailable: (mode) => musicModeIsPlayable(mode, instant, { organisationId: player.organisationId, licensedCatalogueLevel: entitlements.licensedMusicCatalogueLevel })
  });
