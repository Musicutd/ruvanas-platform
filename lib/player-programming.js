...
import { correctionsPlayerInsertions } from "@/lib/corrections-delivery-service";
import {
...
export async function resolvePlayerProgramming(player, instant = new Date(), {
  ...
  // Inside uses the same signed manifest and proof-of-play path, but never
  // inherits a generic campaign, music schedule or public fallback.
  const privateFacility = await prisma.correctionsFacility.findFirst({ where: { locationId: location.id, location: { organisationId: player.organisationId } }, select: { locationId: true } });
  if (privateFacility) {
    const inside = publicAudience ? { insertions: [], activeOverride: null } : await correctionsPlayerInsertions(player, instant);
    return { opening: { isOpen: true, source: "private-facility" },
      resolution: { musicMode: null, reason: "CORRECTIONS_PRIVATE", sourceLabel: "Private Ruvanas Inside", requiredUse: "CORRECTIONS_RADIO" },
      playoutDecision: null, campaignPlayout: { insertions: [] }, schoolPlayout: { insertions: [] }, correctionsPlayout: inside };
  }
