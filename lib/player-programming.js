...
export async function resolvePlayerProgramming(player, instant = new Date(), {
  ...
  if (privateFacility) {
    const insertions = publicAudience ? [] : (await correctionsPlayerInsertions(player, instant)).insertions;
    return { opening: { isOpen: true, source: "private-facility" },
      ...
      playoutDecision: null, campaignPlayout: { insertions: [] }, schoolPlayout: { insertions: [] }, correctionsPlayout: { insertions } };
  }
