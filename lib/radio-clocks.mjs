...
export const RADIO_CLOCK_ITEM_TYPES = Object.freeze([
  "MUSIC_MODE", "MUSIC_TRACK", "PROMO", "VOICE_TRACK", "SHOW_RUNDOWN", "MARKER"
]);
...
function normalizeItem(item, position) {
  ...
  return {
    ...
    voiceTrackSegueId: sourceField === "voiceTrackSegueId" ? sourceId : null,
...
export function reorderRadioClockDraftItems(items, from, to) {
...
export function reviewRadioClockDraft(items = []) {
  const warnings = [];
  const normalized = items.map((item, index) => {
    const durationSeconds = Number(item.durationSeconds);
    const transitionSeconds = Number(item.transitionSeconds);
    if (!Number.isInteger(durationSeconds) || durationSeconds < 0) warnings.push(`Item ${index + 1}: enter a valid duration.`);
    if (!Number.isInteger(transitionSeconds) || transitionSeconds < 0) warnings.push(`Item ${index + 1}: enter a valid mix length.`);
    const duration = Number.isFinite(durationSeconds) && durationSeconds >= 0 ? durationSeconds : 0;
    const overlap = Number.isFinite(transitionSeconds) && transitionSeconds >= 0 ? transitionSeconds : 0;
    if (item.type === "MARKER" && duration !== 0) warnings.push(`Item ${index + 1}: a marker must last zero seconds.`);
    if (item.type !== "MARKER" && duration < 1) warnings.push(`Item ${index + 1}: a playable item needs a positive duration.`);
    if (overlap > 30) warnings.push(`Item ${index + 1}: a mix cannot exceed 30 seconds.`);
    if (["CROSSFADE", "DUCK_VOICE"].includes(item.transition) && (overlap < 1 || overlap >= duration)) {
      warnings.push(`Item ${index + 1}: the mix must be shorter than the item and at least one second.`);
    }
    if (!["CROSSFADE", "DUCK_VOICE"].includes(item.transition) && overlap !== 0) warnings.push(`Item ${index + 1}: this transition cannot overlap another item.`);
    return { ...item, position: index, durationSeconds: duration, transitionSeconds: overlap };
  });
  const timeline = radioClockTimeline(normalized);
  if (timeline.plannedSeconds > RADIO_CLOCK_DURATION_SECONDS) warnings.push(`Clock overruns the hour by ${timeline.plannedSeconds - RADIO_CLOCK_DURATION_SECONDS} seconds.`);
