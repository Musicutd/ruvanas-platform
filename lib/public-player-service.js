    ...
    include: {
      zoneAssignments: {
        ...
        include: {
          zone: {
            include: {
              location: { include: { openingHours: true, openingExceptions: true, correctionsFacility: { select: { locationId: true } } } }
            }
          }
        }
      }
...
export async function loadPublicPlayerStation(database, slug) {
  ...
  return database.station.findFirst({ where: { slug, status: "ACTIVE", publicPlayerEnabled: true, OR: [{ productFamily: null }, { productFamily: { not: "CORRECTIONS" } }] }, include: stationInclude });
}

export function publicPlayerTarget(station, instant = new Date()) {
  if (station?.productFamily === "ONLINE") {
    const channel = station.channels?.[0] || null;
    return channel ? { station, channel, online: true, player: null } : null;
  }
  ...
  if (!channel || !assignment?.zone?.location || assignment.zone.location.correctionsFacility || channel.musicRightsUse === "CORRECTIONS_RADIO") return null;
  return {
    station,
    channel,
    player: {
...
export async function buildPublicPlayerResponse(database, { station, target, sessionId, instant = new Date(), secret }) {
  ...
  if (target.online) {
    ...
    const manifest = {
      version: `online-${station.id}-${station.updatedAt.getTime()}`,
      generatedAt: instant.toISOString(),
      expiresAt: new Date(instant.getTime() + 5 * 60 * 1_000).toISOString(),
      refreshAfterSeconds: PUBLIC_LISTENER_HEARTBEAT_SECONDS,
      state: station.streamConfig?.streamUrl ? "READY" : "NO_PLAYABLE_SOURCE",
      station: { name: station.name, slug: station.slug, description: station.description, logoUrl: station.logoUrl, tagline: station.publicPlayerTagline, accent: station.publicPlayerAccent },
      ...
      playlist: [],
      insertions: [],
      fallbackStream: station.streamConfig?.streamUrl
    ...
    return { ok: true, manifest, programming: null };
  }
...
export function publicPlaybackAssetAllowed(programming, mediaAssetId, instant = new Date()) {
  const eligibleMusic = playableLiveMusicModeEntries(programming.resolution.musicMode, instant, {
    licensedCatalogueLevel: programming.resolution.licensedCatalogueLevel || null,
    requiredUse: programming.resolution.requiredUse || null
  }).some(({ track }) => track.mediaAsset?.id === mediaAssetId);
