...
export function planPublishedTimedChannelSequence({
  ...
  if (active.length !== 1 || !schedule.id || !active[0].id ||
      !Number.isInteger(active[0].version) || active[0].version < 1 ||
      !Array.isArray(active[0].items) || active[0].items.length !== 1) {
    return unavailable("SEQUENCE_PROGRAMME_ARBITRATION_NOT_READY");
  }
  const block = active[0].items[0];
  if (!block.id || block.sourceType !== "MUSIC_MODE" || block.recurrence !== "ONE_OFF" ||
  ...
  return {
    ...
    scheduleId: schedule.id, scheduleVersion: active[0].version,
    programmeItemId: block.id, programmePriority: block.priority,
