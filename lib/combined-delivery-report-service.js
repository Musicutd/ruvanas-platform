...
export async function loadCombinedDeliveryReport(organisationId, input = {}, limit = 2000) {
  ...
  const audioWhere = { organisationId, itemType: "PROMO", occurredAt: { gte: filters.fromInstant, lt: filters.until }, eventType: { in: ["COMPLETED", "FAILED"] }, ...(filters.retailMediaOrderId ? { campaign: { retailMediaOrder: { id: filters.retailMediaOrderId } } } : {}) };
  const visualWhere = { organisationId, occurredAt: { gte: filters.fromInstant, lt: filters.until }, eventType: { in: ["COMPLETED", "FAILED"] }, ...(filters.retailMediaOrderId ? { retailMediaOrderId: filters.retailMediaOrderId } : {}) };
  const [audio, visual, audioCounts, visualCounts, takeoverCompleted, retailMediaVisualCompleted] = await Promise.all([
    prisma.proofOfPlayEvent.findMany({
      where: audioWhere,
      select: { id: true, eventType: true, occurredAt: true, playerName: true, locationName: true, zoneName: true, trackTitle: true, campaign: { select: { name: true, retailMediaOrder: { select: { name: true } } } } },
  ...
  const rows = [
    ...audio.map((event) => ({ medium: "AUDIO", occurredAt: event.occurredAt.toISOString(), eventType: event.eventType, device: event.playerName, location: event.locationName, zone: event.zoneName, content: event.trackTitle, campaignOrOrder: event.campaign?.retailMediaOrder?.name || event.campaign?.name || "", takeover: "" })),
  ...
  return { filters: { from: filters.from, to: filters.to, retailMediaOrderId: filters.retailMediaOrderId }, summary, rows, truncated: audio.length === limit || visual.length === limit };
}
