...
export const SCHOOL_SAFEGUARDING_REVIEW_DECISIONS = Object.freeze([
  "CHANGES_REQUESTED",
  "APPROVED"
]);
...
export function normalizeSchoolSafeguardingReview(input = {}) {
  const decision = String(input.decision || "").trim().toUpperCase();
  if (!SCHOOL_SAFEGUARDING_REVIEW_DECISIONS.includes(decision)) {
    throw new Error("Choose approval or request changes.");
  }
  const notes = optionalText(input.notes);
  if (decision === "CHANGES_REQUESTED" && !notes) {
    throw new Error("Explain the safeguarding changes the school must make.");
  }
  return { decision, notes };
}

export function schoolSafeguardingPolicySnapshot(record = {}) {
  return {
    targetCountries: Array.isArray(record.targetCountries) ? [...record.targetCountries] : [],
    minimumStudentAge: record.minimumStudentAge ?? null,
    maximumStudentAge: record.maximumStudentAge ?? null,
    consentModel: record.consentModel || null,
    studentIdentityMode: record.studentIdentityMode || "DISABLED",
    privacyContactEmail: record.privacyContactEmail || null,
    rawRecordingRetentionDays: record.rawRecordingRetentionDays ?? null,
    consentEvidenceRetentionDays: record.consentEvidenceRetentionDays ?? null,
    localPolicyReference: record.localPolicyReference || null,
    notes: record.notes || null,
    staffModerationConfirmed: Boolean(record.staffModerationConfirmed),
    noDirectMessagingConfirmed: Boolean(record.noDirectMessagingConfirmed),
    privateByDefaultConfirmed: Boolean(record.privateByDefaultConfirmed),
...
export function isSchoolSafeguardingPackLocked(status) {
