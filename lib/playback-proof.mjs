...
function tokenPayload({ playerId, manifestVersion, scheduleItemId, contentId, programmingSource = null }) {
  const base = `${playerId}:${manifestVersion}:${scheduleItemId}:${contentId}`;
  return programmingSource ? `${base}:${programmingSource}` : base;
}
