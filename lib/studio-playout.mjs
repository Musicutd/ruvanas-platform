...
import { catalogueLevelAllows } from "./autodj-genre-entitlements.mjs";
...
export function studioQueueReadiness(asset, { licensedCatalogueEnabled = false, licensedMusicCatalogueEnabled = false, licensedMusicCatalogueLevel = null, planProductFamily = null } = {}) {
  ...
  if (!parseCatalogueTerritories(track.permittedTerritories).ok) return { ready: false, reason: "Choose and approve this catalogue track's territories first." };
  ...
  if (!catalogueLevelAllows(licensedMusicCatalogueLevel, effectiveCatalogueLevel(track.minimumCatalogueLevel))) return { ready: false, reason: "This catalogue track requires a higher plan." };
  const requiredUse = useByProduct[planProductFamily];
  if (!requiredUse || !track.permittedUses?.includes(requiredUse)) return { ready: false, reason: "This catalogue track is not licensed for this Ruvanas pillar." };
  if (asset.licensedCatalogue === true) {
    if (track.rightsReviewStatus !== "APPROVED") return { ready: false, reason: "The licensed track has not passed rights review." };
    if (track.catalogueProvider === "PROMO_ONLY" && !track.distributorItems?.length) return { ready: false, reason: "Provider rights could not be verified." };
    if (track.distributorItems?.some((item) => item.status !== "ACTIVE" || item.autoDjReady !== true || item.canonicalGenre?.active !== true || item.canonicalGenre?.providerReviewStatus !== "APPROVED")) return { ready: false, reason: "The provider track is not available for playback." };
    if (track.distributorItems?.some((item) => !catalogueLevelAllows(licensedMusicCatalogueLevel, item.canonicalGenre?.minimumCatalogueLevel))) return { ready: false, reason: "This genre requires a higher plan." };
  }
