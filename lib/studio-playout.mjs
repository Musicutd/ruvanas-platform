...
export function nextStudioQueuePosition(items = [], area) {
  return 1 + Math.max(-1, ...items.filter((item) => item.area === area && item.status === "READY").map((item) => item.position));
}

export function planStudioFutureReorder(items = [], itemId, targetPosition) {
  const future = items.filter((item) => item.area === "LIVE" && item.status === "READY")
    .sort((left, right) => left.position - right.position || new Date(left.createdAt || 0) - new Date(right.createdAt || 0) || String(left.id).localeCompare(String(right.id)));
  const from = future.findIndex((item) => item.id === itemId);
  if (from < 0) throw new Error("Only a future playlist item can be moved.");
  if (future[from].locked) throw new Error("Unlock this future item before moving it.");
  if (!Number.isInteger(targetPosition) || targetPosition < 0 || targetPosition >= future.length) throw new Error("Choose a valid future playlist position.");
  if (future.slice(Math.min(from, targetPosition), Math.max(from, targetPosition) + 1).some((item) => item.locked)) {
    throw new Error("Unlock the intervening items before moving past them.");
  }
  const [moving] = future.splice(from, 1);
  future.splice(targetPosition, 0, moving);
  return future.map((item, position) => ({ id: item.id, position })).filter((item, position) => items.find((candidate) => candidate.id === item.id)?.position !== position);
}

export function planStudioFutureReplacement(items = [], itemId, replacementItemId) {
  const target = items.find((item) => item.id === itemId && item.area === "LIVE" && item.status === "READY");
  if (!target) throw new Error("Only a future Manual queue item can be replaced.");
  if (target.locked) throw new Error("A locked future item cannot be replaced.");
  if (target.estimatedStartAt && new Date(target.estimatedStartAt).getTime() <= Date.now()) throw new Error("This item's estimated start is no longer in the future. Refresh the queue.");
  const replacement = items.find((item) => item.id === replacementItemId && item.area === "PREPARE" && item.status === "READY");
  if (!replacement) throw new Error("Choose a ready item from the private Prepare area.");
  if (replacement.locked) throw new Error("Unlock the prepared item before using it as a replacement.");
  return {
    outgoing: { id: target.id, area: "PREPARE", position: nextStudioQueuePosition(items, "PREPARE"), estimatedStartAt: null },
    incoming: { id: replacement.id, area: "LIVE", position: target.position, estimatedStartAt: target.estimatedStartAt || null }
  };
}
