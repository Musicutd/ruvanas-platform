export const STUDIO_PRODUCT_DESTINATIONS = Object.freeze({
  ...
  ORGANISATIONS_ANNOUNCEMENT: Object.freeze({ product: "ORGANISATIONS", label: "Organisation announcement", description: "Open governed announcements with this approved master selected.", entitlement: "organisationsEnabled" }),
  ORGANISATIONS_PODCAST: Object.freeze({ product: "ORGANISATIONS", label: "Organisation podcast", description: "Open Organisations podcasts with this approved master selected.", entitlement: "organisationsEnabled" }),
  ORGANISATIONS_EVENT: Object.freeze({ product: "ORGANISATIONS", label: "Organisation event", description: "Open Event Mode with this approved master selected.", entitlement: "organisationsEnabled" })
});
...
export function studioWorkflowPath({ destination, promoVersionId, mediaAssetId, targetEpisodeId = null }) {
  ...
  if (destination === "ORGANISATIONS_ANNOUNCEMENT") return `/dashboard/organisations?tab=announcements&mediaAssetId=${encodeURIComponent(mediaAssetId)}`;
  if (destination === "ORGANISATIONS_PODCAST") return `/dashboard/podcasts?product=ORGANISATIONS&mediaAssetId=${encodeURIComponent(mediaAssetId)}`;
  if (destination === "ORGANISATIONS_EVENT") return `/dashboard/organisations?tab=events&mediaAssetId=${encodeURIComponent(mediaAssetId)}`;
