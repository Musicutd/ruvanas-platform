...
export function buildStudioMasteringFilters(value) {
  ...
  const filters = [`loudnorm=I=${mastering.targetLufs}:TP=${mastering.truePeakDbfs}:LRA=${mastering.maxLoudnessRangeLu}`];
  if (mastering.limiter) filters.push(`alimiter=limit=${Math.pow(10, mastering.truePeakDbfs / 20).toFixed(4)}`);
  return filters;
}
