...
export function normalizeEditorState(value = {}) {
  ...
  const effects = normalizeStudioEffects(value.effects);
  const mastering = normalizeStudioMastering(value.mastering, value);
  return {
    clips: cleanClips,
    markers,
...
export function changeSelectionGain(clips, startMs, endMs, adjustmentDb, idFactory = () => crypto.randomUUID()) {
  const start = Math.min(Number(startMs), Number(endMs));
  const end = Math.max(Number(startMs), Number(endMs));
  const adjustment = Number(adjustmentDb);
  if (!(end > start) || !Number.isFinite(adjustment) || adjustment === 0) return clips;
  const split = splitAt(splitAt(clips, start, idFactory), end, idFactory);
  if (split.length > MAX_EDITOR_CLIPS) throw new Error("This edit would exceed the waveform clip limit. Save or simplify the timeline first.");
  return split.map((clip) => {
    if (clip.locked || clip.kind === "SILENCE" || clip.timelineStartMs < start || clip.timelineStartMs >= end) return clip;
    return { ...clip, gainDb: clamp(Number(clip.gainDb || 0) + adjustment, -36, 18) };
  });
}
...
export function waveformUsesProFeatures(value = {}) {
  return (value.markers || []).length > 0 || (value.clips || []).some((clip) =>
    Number(clip.gainDb || 0) !== 0 || clip.fadeInCurve !== "linear" || clip.fadeOutCurve !== "linear"
  );
}

export function assertStudioWaveformWriteAllowed(value, entitlements = {}) {
  if (entitlements?.studioLevel !== "PRO" && waveformUsesProFeatures(value)) {
    throw new Error("Studio Pro is required for gain, markers, named regions, and advanced fade curves. Existing Pro edit decisions remain preserved.");
  }
  return true;
}
