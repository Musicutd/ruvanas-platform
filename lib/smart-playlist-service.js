...
const candidateInclude = cataloguePlaybackTrackInclude;
...
export async function listSmartPlaylists(organisationId) {
  const playlists = await prisma.smartPlaylist.findMany({
    where: { organisationId, simpleBuildMode: null, status: { not: "ARCHIVED" } },
...
async function playlistForEvaluation(client, organisationId, smartPlaylistId) {
  return client.smartPlaylist.findFirst({
    where: { id: smartPlaylistId, organisationId, simpleBuildMode: null, status: { not: "ARCHIVED" } },
    include: playlistInclude
  });
}
...
async function evaluateWithClient(client, playlist, instant) {
  ...
  const [tracks, licensedCatalogueLevel] = await Promise.all([
    client.track.findMany({ ...query, include: candidateInclude }),
    catalogueLevelForOrganisation(client, playlist.organisationId)
  ]);
  return evaluateSmartPlaylistCandidates(tracks, playlist, instant, licensedCatalogueLevel);
}
...
export async function archiveSmartPlaylist({ organisationId, smartPlaylistId, actorUserId }) {
  return prisma.$transaction(async (tx) => {
    const playlist = await tx.smartPlaylist.findFirst({ where: { id: smartPlaylistId, organisationId, simpleBuildMode: null }, select: { id: true, musicModeId: true, status: true } });
    if (!playlist) return null;
    const updated = await tx.smartPlaylist.update({
      where: { id: playlist.id },
