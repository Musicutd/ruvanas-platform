...
import { correctionsRenderEvidence, correctionsReviewTransition, correctionsSchedulingGate, CORRECTIONS_REVIEW_POLICY_VERSION, normalizeCorrectionsProgramme } from "@/lib/corrections-workflow.mjs";
...
const renderInclude = { outputMediaAsset: true, outputPromoVersion: true, project: { select: { organisationId: true, createdByUserId: true, currentVersion: true, title: true } } };
...
export async function reviewCorrectionsProgramme(access, programmeId, { stage, decision, note }) {
  return runSerializableTransaction(prisma, async (tx) => {
    ...
    if (!render || correctionsRenderEvidence(render).fingerprint !== submission.sourceFingerprint) throw new Error("The submitted audio has changed. Create a new reviewed version.");
...
export async function correctionsProgrammeReadiness(access, programmeId) {
  ...
  return { programmeId, ...correctionsSchedulingGate({ programme, submission, reviews: submission?.reviews || [], ...policies, render }), playbackEnabled: false };
}
