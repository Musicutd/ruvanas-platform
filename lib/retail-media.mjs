...
const TARGET_TYPES = new Set(["BRAND", "LOCATION_GROUP", "ZONE", "STATION", "CHANNEL"]);
...
export function normaliseInventoryTargets(input) {
  ...
  const targets = input.map((item) => {
    ...
    return {
      ...
      stationId: targetType === "STATION" ? targetId : null,
      channelId: targetType === "CHANNEL" ? targetId : null
    };
  });
  const keys = targets.map((target) => `${target.targetType}:${target.brandId || target.locationGroupId || target.zoneId || target.stationId || target.channelId}`);
