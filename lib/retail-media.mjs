...
export const RETAIL_MEDIA_REPORTING_NOTICE =
  "Audio plays are device-confirmed delivery events. They are not listeners, viewers, impressions or proof that media caused an operational outcome.";
...
export function normaliseRetailMediaOrder(input = {}) {
  const creativeIds = Array.isArray(input.creativePromoVersionIds)
    ...
    : [];
  if (creativeIds.length < 1 || creativeIds.length > 20) {
    throw new Error("Choose between 1 and 20 approved promo versions for the order.");
  }
...
export function retailMediaOrderApprovalBlockers(order, now = new Date()) {
  ...
  if (!Array.isArray(order?.creatives) || order.creatives.length === 0) blockers.push("The order needs at least one creative.");
  if (order?.creatives?.some((creative) => creative.status !== "APPROVED")) {
