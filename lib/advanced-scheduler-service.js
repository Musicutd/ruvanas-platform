...
const playbackModeInclude = { tracks: { include: { track: { include: cataloguePlaybackTrackInclude } } } };
...
export async function programmeSchedulerSources(organisationId, instant = new Date()) {
  const [channels, musicModes, radioClocks, rundowns, licensedCatalogueLevel] = await Promise.all([
    prisma.channel.findMany({ where: { organisationId, status: "ACTIVE" }, select: { id: true, name: true, station: { select: { id: true, name: true } }, programmeSchedule: { select: { id: true } } }, orderBy: { name: "asc" }, take: 100 }),
  ...
  return {
    channels: channels.map((channel) => ({ id: channel.id, stationId: channel.station?.id || null, name: channel.station ? `${channel.station.name} / ${channel.name}` : channel.name, configured: Boolean(channel.programmeSchedule) })),
...
export async function validateProgrammeScheduleSources(client, organisationId, items, { requireReady = false, instant = new Date() } = {}) {
  ...
  const [musicModes, radioClocks, rundowns, licensedCatalogueLevel] = await Promise.all([
  ...
  for (const item of items) {
    ...
    if (item.sourceType === "MUSIC_MODE" && !musicModeIsPlayable(source, instant, { organisationId, requiredUse: "ONLINE_RADIO", licensedCatalogueLevel })) throw new Error(`${item.label} needs an active Music Mode with playable Online Radio tracks.`);
...
export async function publishProgrammeScheduleVersion({ organisationId, scheduleId, versionId, actorUserId, conflictsAcknowledged = false, instant = new Date() }) {
  return prisma.$transaction(async (tx) => {
    ...
    if (!version) return null;
    await assertCorrectionsSchedulingAllowed(tx, { organisationId, channelId: version.schedule.channelId });
