...
export const studioWaveformEditorInclude = {
  ...
  renders: { orderBy: { createdAt: "desc" }, take: 10, include: { version: { select: { version: true } }, outputMediaAsset: { select: { id: true, name: true, durationSeconds: true } } } }
};
...
export function serializeStudioWaveformProject(project, entitlements, { restrictedMediaPath = null } = {}) {
  return {
    ...
    renders: project.renders.map((render) => ({
      id: render.id, status: render.status, preset: render.preset, versionNumber: render.version.version, loudnessLufs: render.loudnessLufs,
