...
import { applyRadioAdvertisingPolicy, campaignHasRadioTargets } from "./radio-advertising.mjs";
...
function targetMatchesPlayer(campaign, player) {
  ...
  const channelId = player.channelId || player.zone.channelAssignments?.[0]?.channelId || null;
  const stationId = player.stationId || player.zone.channelAssignments?.[0]?.channel?.stationId || null;
  return (campaign.targets || []).some((target) =>
    ...
    (target.targetType === "STATION" && target.stationId === stationId) ||
    (target.targetType === "CHANNEL" && target.channelId === channelId)
  );
}
    ...
    for (let cursor = bucketStart.getTime(); cursor < expiresAt.getTime(); cursor += 60_000) {
      ...
      for (const schedule of campaign.schedules || []) {
        ...
        candidates.push({
          ...
          radioAdvertising: campaignHasRadioTargets(campaign),
          plannedStart,
