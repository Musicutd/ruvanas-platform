...
function resolveGenres(value, index) {
  const source = decodeSpreadsheetText(value);
  if (!source) return { genreIds: [], genreNames: [], notes: [] };
  ...
  return { genreIds: [...selected.keys()].slice(0, 10), genreNames: [...selected.values()].map((genre) => genre.name), notes };
}

export async function parseCatalogueManifest({ buffer, fileName, genres }) {
  ...
  for (let rowIndex = headerRowIndex + 1; rowIndex < rawRows.length; rowIndex += 1) {
    ...
    const resolvedGenres = resolveGenres(row.genreSource, genreIndex);
