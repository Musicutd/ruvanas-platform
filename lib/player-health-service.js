    ...
    try {
      await prismaClient.$transaction(async (tx) => {
        ...
        await enqueueNotificationEvent(tx, {
          organisationId: player.organisationId,
          type: "PLAYER_OFFLINE",
          severity: notificationSeverityForIncident(incident.severity),
          title: "Player offline",
          message: `${player.name} stopped reporting and requires an operational check.`,
          entityType: "PlayerHealthIncident",
          entityId: incident.id,
          metadata: { playerId: player.id, locationId: player.zone.location.id, zoneId: player.zone.id },
          dedupeKey: `player-offline:${incident.id}`,
          correlationId: `player-health:${incident.id}`,
          occurredAt: now
        });
      });
      created += 1;
