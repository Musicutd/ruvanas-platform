const subscriberSections = [
  {
    ...
    description: "Set up stations, shop players and the live devices using your plan.",
    items: [
      { id: "station", label: "Radio station", description: "Open your station and review its streaming setup." },
      { id: "players", href: "/dashboard/players", label: "Shop players", description: "Prepare and check one secure player for each shop." },
      { id: "sessions", href: "/dashboard/player-sessions", label: "Live shop streams", description: "See which players are currently using your stream allowance." }
    ]
  },
  {
    id: "content",
...
export function resolveDashboardNextAction({
  ...
  if (configuredPlayerCount === 0) {
    return {
      code: "SET_UP_PLAYER",
      eyebrow: "NEXT STEP",
      title: "Prepare your first shop player",
      description: "Create a secure player for the shop or playback area where customers will hear your radio.",
      href: "/dashboard/players",
      label: "Set up a shop player"
    };
  }
  ...
  if (activePlayerStreams === 0) {
    return {
      ...
      title: "Bring a shop player online",
      description: "Follow the guided player checklist and confirm that the shop is receiving audio.",
      href: "/dashboard/players",
