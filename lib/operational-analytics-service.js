...
async function loadSchoolAggregate(organisationId) {
  const profile = await prisma.schoolProfile.findUnique({ where: { organisationId }, select: { id: true } });
  if (!profile) return null;
  const expiryCutoff = new Date(Date.now() + 30 * 24 * 60 * 60 * 1_000);
  const [programmes, episodes, assignments, submissions, assessments, portfolios, pendingReviews, consentActions] = await Promise.all([
