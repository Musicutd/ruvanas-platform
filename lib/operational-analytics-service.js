...
import { buildStationBreakdown, buildSubscriberBreakdowns } from "@/lib/subscriber-insights.mjs";
...
export async function loadOperationalAnalyticsReport(organisationId, inputFilters = {}, aggregation = null) {
  ...
  const [hourRows, dimensionRows, players, storage, content, school, stationRows] = await Promise.all([
    ...
    prisma.analyticsHourlyAggregate.groupBy({
      by: ["playerId", "playerName", "locationId", "locationName", "zoneId", "zoneName"],
      where: { organisationId, bucketStart: { gte: window.from, lt: window.until } },
      _sum: Object.fromEntries(COUNT_FIELDS.map((field) => [field, true]))
    }),
    prisma.player.findMany({
      ...
      select: {
        ...
        zone: { select: { name: true, location: { select: { name: true } } } }
      },
      orderBy: { name: "asc" }
    }),
  ...
  const channelIds = stationRows.map((row) => row.channelId).filter(Boolean);
  const channels = channelIds.length ? await prisma.channel.findMany({
    where: { organisationId, id: { in: channelIds } },
    select: { id: true, station: { select: { id: true, name: true, status: true } } }
  }) : [];
  const breakdowns = buildSubscriberBreakdowns(dimensionRows);
  return {
    filters,
    summary,
    days,
    players: {
      ...
      items: enabledPlayers.map((player) => ({
        id: player.id,
        name: player.name,
        status: player.status,
        online: Boolean(player.lastHeartbeatAt && player.lastHeartbeatAt.getTime() >= onlineCutoff),
    ...
    contentMix: [
      { id: "music", label: "Music", value: summary.musicCompletedCount },
      { id: "promotions", label: "Promotions", value: summary.promoCompletedCount },
      { id: "school", label: "School announcements", value: summary.schoolCompletedCount }
    ],
    breakdowns: { ...breakdowns, stations: buildStationBreakdown(stationRows, channels) },
    school,
    aggregation,
