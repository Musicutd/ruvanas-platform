...
export function studioMultitrackTrackLimit(entitlements = {}) {
  return entitlements?.studioLevel === "PRO" || Number(entitlements?.planTierNumber) >= 3
    ? MAX_MULTITRACK_TRACKS
    : BASIC_MULTITRACK_TRACKS;
}

export function multitrackUsesProFeatures(value = {}) {
  return (value.tracks || []).some((track) =>
    Number(track.gainDb || 0) !== 0 || Number(track.pan || 0) !== 0 || track.solo === true ||
    track.armed === true || track.locked === true || (track.automation || []).length > 0 ||
    (track.clips || []).some((clip) => Number(clip.gainDb || 0) !== 0)
  );
}

export function assertStudioMultitrackWriteAllowed(value, entitlements = {}) {
  const limit = studioMultitrackTrackLimit(entitlements);
  if ((value?.tracks || []).length > limit) throw new Error(`Your Studio ${entitlements?.studioLevel === "PRO" ? "Pro" : "Basic"} plan supports up to ${limit} tracks.`);
  if (entitlements?.studioLevel !== "PRO" && multitrackUsesProFeatures(value)) {
    throw new Error("Studio Pro is required to change Track Gain, Clip Gain, Pan, Solo, Record Arm, Track Lock, or gain automation. Existing Pro project data remains preserved.");
  }
  return true;
}
...
export function splitMultitrackClip(value, { trackId, clipId, timelinePositionMs, idFactory = () => crypto.randomUUID() }, options = {}) {
  const state = normalizeMultitrackState(value, options);
  const track = state.tracks.find((item) => item.clientId === trackId);
  const clip = track?.clips.find((item) => item.clientId === clipId);
  if (!track || !clip) throw new Error("Choose a clip on the timeline.");
  if (track.locked || clip.locked) throw new Error("Unlock the track and clip before using Blade.");
  const local = Math.round(Number(timelinePositionMs) - clip.timelineStartMs);
  if (local <= 0 || local >= clipDuration(clip)) throw new Error("Place Blade inside the selected clip.");
  const splitSource = clip.sourceStartMs + local;
  const left = { ...clip, clientId: idFactory(), sourceEndMs: splitSource, fadeOutMs: 0 };
  const right = { ...clip, clientId: idFactory(), sourceStartMs: splitSource, timelineStartMs: Number(timelinePositionMs), fadeInMs: 0 };
  return { ...state, tracks: state.tracks.map((item) => item.clientId !== track.clientId ? item : { ...item, clips: item.clips.flatMap((candidate) => candidate.clientId === clip.clientId ? [left, right] : [candidate]).sort((a, b) => a.timelineStartMs - b.timelineStartMs) }) };
}
