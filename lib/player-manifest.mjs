...
export function buildPlayerManifest({ player, resolution, campaignPlayout = null, schoolPlayout = null, proofSecret, instant = new Date() }) {
  ...
  const rotation = deterministicWeightedRotation(playableEntries(mode, instant), seed);
  ...
  return {
    ...
    insertions: insertionItems.map((item) => ({
      ...
      title: item.itemType === "SCHOOL_ANNOUNCEMENT" ? item.announcementTitle : item.promoName,
      artist: item.itemType === "SCHOOL_ANNOUNCEMENT" ? "School announcement" : "Promotion",
