...
import { orderedMusicModeEntries, playableLiveMusicModeEntries } from "./music-mode-playback.mjs";
import {
...
export function buildPlayerManifest({
  player,
  resolution,
  playoutDecision = null,
  campaignPlayout = null,
  schoolPlayout = null,
  correctionsPlayout = null,
  proofSecret,
  listenerToken = null,
  instant = new Date(),
  mediaUrlFor = (mediaAssetId, token) => appendPlayerListenerToken(`/api/player/media/${mediaAssetId}`, token),
  liveUrlFor = (sourceId, token) => appendPlayerListenerToken(`/api/player/live/${sourceId}`, token),
  includeProof = true
}) {
  const bucketMs = PLAYER_MANIFEST_TTL_SECONDS * 1000;
  const bucketStart = Math.floor(instant.getTime() / bucketMs) * bucketMs;
  const plannedExpiry = new Date(bucketStart + bucketMs);
  const expiresAt = resolution.validUntil instanceof Date && resolution.validUntil < plannedExpiry ? resolution.validUntil : plannedExpiry;
  ...
  const playableEntries = playableLiveMusicModeEntries(mode, instant, {
    licensedCatalogueLevel: resolution.licensedCatalogueLevel || null,
    requiredUse: resolution.requiredUse || null
  });
  const rotation = orderedMusicModeEntries(mode, playableEntries) || deterministicWeightedRotation(playableEntries, seed);
  ...
  const correctionsItems = correctionsPlayout?.insertions || [];
  ...
  const externalLiveSource = resolution.liveSource || null;
  ...
  return {
    ...
    refreshAfterSeconds: Math.max(1, Math.min(resolution.reason === "CORRECTIONS_PRIVATE" ? 5 : PLAYER_MANIFEST_REFRESH_SECONDS, Math.floor((expiresAt.getTime() - instant.getTime()) / 1000))),
    player: {
      id: player.id,
      name: player.name,
    ...
    state: externalLiveSource || musicItems.length || insertionItems.length
      ? "READY"
      : mode ? "NO_PLAYABLE_TRACKS" : resolution.reason,
    schedule: mode || externalLiveSource ? {
    ...
    activeOverride: correctionsPlayout?.activeOverride || null,
    ...
    externalLive: externalLiveSource ? {
      sourceId: externalLiveSource.id,
      providerKey: externalLiveSource.providerKey,
      sourceLabel: resolution.sourceLabel || "External Live",
      mediaUrl: liveUrlFor(externalLiveSource.id, listenerToken)
    } : null,
    live,
    playlist: musicItems.map(({ weight, track, scheduleItemId }, index) => ({
      ...
      weight,
      mediaUrl: mediaUrlFor(track.mediaAsset.id, listenerToken),
      ...(includeProof ? { proofToken: createPlaybackProofToken({
        playerId: player.id,
    ...
    insertions: insertionItems.map((item) => ({
      ...
      trackId: item.itemType === "MUSIC" ? item.trackId : undefined,
      title: item.programmingSource?.startsWith("CORRECTIONS_") ? item.title : item.itemType === "SCHOOL_ANNOUNCEMENT" ? (item.displayTitle || item.announcementTitle) : item.promoName,
      artist: item.programmingSource?.startsWith("CORRECTIONS_") ? item.artist : item.itemType === "SCHOOL_ANNOUNCEMENT" ? (item.displayArtist || "School announcement") : "Promotion",
      durationSeconds: item.durationSeconds,
      plannedStart: item.plannedStart.toISOString(),
      hardStart: item.programmingSource?.startsWith("CORRECTIONS_") || item.itemType === "SCHOOL_ANNOUNCEMENT" ? true : item.exactTimeHardStart,
      mandatory: item.programmingSource?.startsWith("CORRECTIONS_") || item.itemType === "SCHOOL_ANNOUNCEMENT" ? true : item.mandatory,
      priority: item.programmingSource?.startsWith("CORRECTIONS_") || item.itemType === "SCHOOL_ANNOUNCEMENT" ? "VERY_HIGH" : item.priority,
      programmingSource: item.programmingSource || (item.itemType === "SCHOOL_ANNOUNCEMENT" ? "SCHOOL_PROGRAMMING" : "CAMPAIGN"),
      mediaUrl: mediaUrlFor(item.mediaAssetId, listenerToken),
      ...(includeProof ? { proofToken: createPlaybackProofToken({
        playerId: player.id,
        manifestVersion: version,
        scheduleItemId: item.scheduleItemId,
        contentId: item.trackId || item.promoVersionId || item.mediaAssetId
      }, proofSecret) } : {}),
      ...(includeProof ? {
      programmingSourceProofToken: createPlaybackProofToken({
        playerId: player.id,
