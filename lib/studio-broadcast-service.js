...
import { studioManualOutputAvailability } from "./studio-playout.mjs";
...
export async function scanStudioBroadcastConnections(database, { now = new Date() } = {}) {
  ...
  if (!studioManualOutputAvailability().connected) {
    // A healthy Centova listener endpoint is not evidence that Studio is its
    // source. Keep legacy Studio links honest without interrupting station AutoDJ.
    const destinationIds = [...new Set(links.map((link) => link.destinationId))];
    if (links.length) await database.$transaction([
      database.studioBroadcastSessionDestination.updateMany({ where: { OR: links.map((link) => ({ sessionId: link.sessionId, destinationId: link.destinationId })) }, data: { state: "STANDBY", nextReconnectAt: null, lastSafeError: "Studio programme output is not connected to this destination." } }),
      database.studioBroadcastDestination.updateMany({ where: { id: { in: destinationIds } }, data: { connectionState: "STANDBY", listenerCount: null, listenerTelemetryAt: null, nextReconnectAt: null } })
    ]);
    return { ...result, waitingForOutput: links.length };
  }
...
export async function refreshStudioBroadcastMetadata(database) {
  if (!studioManualOutputAvailability().connected) return { scanned: 0, updated: 0, externalUpdates: 0, metadataFailures: 0 };
