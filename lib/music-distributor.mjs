...
const territory = z.string().trim().toUpperCase().regex(/^(?:[A-Z]{2}|WORLDWIDE)$/);
...
export function parseDistributorCataloguePage(input, connection = {}) {
  ...
  try {
    ...
    const collections = parsed.data.collections.map((item) => withMetadataChecksum({
      ...
      permittedTerritories: defaults(item.permittedTerritories, connection.defaultPermittedTerritories),
    ...
    const tracks = parsed.data.tracks.map((item) => {
      ...
      return withMetadataChecksum({
        ...
        permittedTerritories: defaults(item.permittedTerritories, defaults(collectionTerritories, connection.defaultPermittedTerritories)),
...
export function distributorTrackDecision(track, { catalogueLevel: subscriberLevel, territory: requestedTerritory, rightsUse: requestedUse, instant = new Date() } = {}) {
  ...
  const territories = new Set((track.permittedTerritories || []).map((item) => item.toUpperCase()));
  if (requestedTerritory && !territories.has("WORLDWIDE") && !territories.has(String(requestedTerritory).toUpperCase())) return { playable: false, reason: "TERRITORY_NOT_PERMITTED" };
