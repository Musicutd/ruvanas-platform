...
export const REGISTRATION_PRODUCT_CONTENT = Object.freeze([
  ...
  Object.freeze({ id: "CORRECTIONS", slug: "corrections", label: "Ruvanas Inside", shortLabel: "Inside", description: "Secure, internal radio and rehabilitation media for correctional facilities. Private delivery and governance require facility setup." })
]);
...
function databaseRegistrationPlan(defaultPlan, databasePlan) {
  if (!databasePlan) return defaultPlan;
  return {
    ...defaultPlan,
    name: databasePlan.name,
    tierNumber: databasePlan.tierNumber,
    priceLabel: formatMonthlyPlanPrice(databasePlan.monthlyPriceCents, { from: databasePlan.tierNumber === 5 }),
    stationLimit: databasePlan.stationLimit,
    storageLimitGb: databasePlan.storageLimitGb,
    listenerLimit: databasePlan.listenerLimit,
    maxBitrateKbps: databasePlan.maxBitrateKbps,
    studioLevel: databasePlan.tierNumber >= 3 ? "Pro" : "Basic",
    digitalSignageDisplayLimit: databasePlan.productFamily === "RETAIL" && databasePlan.digitalSignageEnabled ? databasePlan.stationLimit : null,
    licensedMusicCatalogueLevel: databasePlan.licensedMusicCatalogueLevel,
    catalogueDescription: catalogueDescription(databasePlan),
    enterpriseContactRequired: databasePlan.tierNumber === 5,
    active: databasePlan.active,
    promoUploadEnabled: databasePlan.promoUploadEnabled,
    retailRadioEnabled: databasePlan.retailRadioEnabled,
    schoolRadioEnabled: databasePlan.schoolRadioEnabled,
    onlineRadioEnabled: databasePlan.onlineRadioEnabled,
    healthRadioEnabled: databasePlan.healthRadioEnabled,
    faithRadioEnabled: databasePlan.faithRadioEnabled,
    organisationsEnabled: databasePlan.organisationsEnabled,
    correctionsRadioEnabled: databasePlan.correctionsRadioEnabled,
    schoolPublicPublishingEnabled: databasePlan.schoolPublicPublishingEnabled,
    retailMediaEnabled: databasePlan.retailMediaEnabled,
    digitalSignageEnabled: databasePlan.digitalSignageEnabled
  };
}
...
export function registrationPlanOptions(product) {
  ...
  return publicPlansForProduct(normalized).map((plan) => ({
    ...
    studioLevel: plan.tierNumber >= 3 ? "Pro" : "Basic",
    digitalSignageDisplayLimit: plan.productFamily === "RETAIL" && plan.digitalSignageEnabled ? plan.stationLimit : null,
...
export function registrationProductsFromDatabasePlans(databasePlans = []) {
  const databaseByCode = new Map(
    databasePlans
      .filter((plan) => plan && plan.publiclyAvailable)
      .map((plan) => [plan.code, plan])
  );
  return REGISTRATION_PRODUCT_CONTENT.map((product) => ({
    ...product,
    plans: registrationPlanOptions(product.id)
      .map((plan) => databaseRegistrationPlan(plan, databaseByCode.get(plan.code)))
      .filter((plan) => databaseByCode.has(plan.code) ? databaseByCode.get(plan.code).active : true)
...
export function resolveRegistrationDeepLink({ platform, tier } = {}) {
  ...
  const selectablePlan = plan || null;
  return {
    product: product.id,
