...
export function buildDigitalSignageManifest({ device, playlists = [], takeovers = [], proofSecret, instant = new Date() }) {
  ...
  const signatureInput = playlist
    ? [device.id, takeover?.id || "scheduled", playlist.id, playlist.version, bucketStart, ...playlist.items.map((item) => `${item.id}:${item.assetId}:${item.asset.checksumSha256}:${item.durationSeconds}`)].join("|")
