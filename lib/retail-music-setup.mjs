export const RETAIL_CATALOGUE_MODE = "approved-catalogue";
...
export function buildRetailMusicAreas(programming = {}) {
  ...
  return (data.targets || [])
    ...
    .map((target) => {
      ...
      const canPrepareChannel = !target.channelId && Array.isArray(target.channelIds) && target.channelIds.length === 0;
...
export function retailMusicSelection(area, musicModes = []) {
  ...
  return {
    modeId: area?.channel?.autoDjPolicy?.catalogueRotation
      ? RETAIL_CATALOGUE_MODE
      : playableModes.some((mode) => mode.id === currentModeId) ? currentModeId : playableModes[0]?.id || "",
