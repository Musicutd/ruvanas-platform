...
export async function listAutoDjTargets(organisationId, entitlements) {
  const [locations, channels] = await Promise.all([
    entitlements.retailRadioEnabled || entitlements.schoolRadioEnabled || entitlements.healthRadioEnabled || entitlements.faithRadioEnabled || entitlements.organisationsEnabled ? prisma.location.findMany({
    ...
    entitlements.onlineRadioEnabled || entitlements.schoolRadioEnabled || entitlements.healthRadioEnabled || entitlements.faithRadioEnabled || entitlements.organisationsEnabled ? prisma.channel.findMany({
      where: { organisationId, status: "ACTIVE" },
  ...
  if (entitlements.organisationsEnabled) {
    for (const channel of channels.filter((item) => item.station?.productFamily === "ORGANISATIONS")) targets.push({ id: channel.id, type: "ORGANISATIONS_CHANNEL", label: `${channel.station?.name ? `${channel.station.name} / ` : ""}${channel.name}`, timezone: channel.programmeSchedule?.timezone || "Europe/Malta", territory: null, rightsUse: "ORGANISATIONS_RADIO", product: "ORGANISATIONS", channelId: channel.id });
  }
  return targets;
}
