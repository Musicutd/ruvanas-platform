...
test("webhook endpoint validation blocks local networks and unsafe URL features", () => {
  ...
  for (const value of ["http://partner.example/hook", "https://localhost/hook", "https://127.0.0.1/hook", "https://10.0.0.5/hook", "https://user:pass@partner.example/hook", "https://partner.example:8443/hook"]) {
...
test("event subscriptions are allow-listed and deduplicated", () => {
  assert.deepEqual(normalizeWebhookEventTypes(["proof.accepted", "PROOF.ACCEPTED", "unknown"]), ["proof.accepted"]);
});
